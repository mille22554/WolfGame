...
| 事件 | 行為 |
|------|------|
| `ADVANCE_DAY`（內部） | `daySummaries.push(summarizeDay)`；day++；清空 nightActions/voteReady/wolfReady/wolfDiscussionLog/skippedHumans/idleCounts；刪除 night 相容暫存（wolfKillTarget/guardProtectedTarget/seerCheckTarget/seerCheckResult）；boardVersion++ → **NIGHT_DISCUSSION_OPEN**（新一天） |
