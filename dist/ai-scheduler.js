...
import { buildPreSpeechPrompt, buildJudgePrompt, buildExpandPrompt, summarizeDay, buildWolfPreSpeechPrompt, summarizeWolfDiscussion, } from './character-session.js';
...
function findTextTarget(raw) {
    ...
    while ((m = WOLF_TARGET_RE.exec(raw)) !== null) {
        ...
        if (window.includes('不') || window.includes('別'))
            continue;
        return m;
    }
    return null;
...
export function findEmptyDiscussionFabrication(text) { const m = /(大家|眾人)(討論|說|提|講|發言).{0,6}(了|過|一下|一些)/.exec(text); return m ? m[0] : ''; }
...
export function checkWolfDraft(raw, st, pid) {
    ...
    if (first.kind === 'coherence')
        return { kind: 'coherence', hit: first.hit, note: `你的同意目標（${first.hit.replace('同意接地失敗（', '').replace('）', '')}）不在白板紀錄中。同意只能針對上面已出現的編號；新目標要當新提案講（「我覺得殺P編號」）。` };
