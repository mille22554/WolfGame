...
function taskInstruction(kind: PromptKind, playerId: number): string {
  switch (kind) {
    case 'speech':
      return `【任務】你是 P${playerId}，請進行白天發言（一句話，30-60字）。圍繞「誰的反應讓你在意」「想聽聽誰的說法」聊，用「我比較在意…」語氣，避免直接定罪。像一般人自然講話，不要套用角色口頭禪或壓力台詞，語氣不要強烈或浮誇。格式：P${playerId}：「你的發言」`;
    case 'vote':
    ...
    case 'wolf_speech':
      return `【任務】你是 P${playerId}（人狼），請與同伴討論今晚要襲擊誰、協調目標（一句話，30-60字）。直接指名具體目標（P編號）並說理由，如「P3 話多可能是占卜師，先殺他」（僅當討論紀錄中真有此觀察時才可用行為理由；沒有材料時誠實說直覺或隨機即可，不要編造理由）。你的同盟列在【你的角色資訊】，襲擊同盟是規則上不可能的行為，絕對不要考慮。守衛保護誰、誰是甚麼職業都是秘密，無從得知：不得聲稱知道，也不得以任何守衛相關猜測（無論「會保護P編號」或「沒有保護跡象」）作為選擇或排除目標的理由。請使用繁體中文。用「我覺得今晚…」語氣。像一般人自然講話，不要套用角色口頭禪或壓力台詞，語氣不要強烈或浮誇。格式：P${playerId}：「你的發言」；結尾另起一行附加決策旗標[決定:殺P編號]（已決定目標時）或[決定:資訊不足]（尚無法決定時），只可附加其一。`;
  }
}
...
export function buildExpandPrompt(state: GameState, playerId: number, preSpeech: string): string {
  ...
  return `${base}\n\n【你的預發言草稿】${preSpeech}\n請把這則草稿改寫成自然的口語發言（30-60 字，像一般人在會議中講話），不必保留草稿的字句與句構，也不要套用任何固定句式。鐵則：草稿的核心論點不得改變，不得新增草稿中沒有的理由或觀察。`;
}
...
function emptyBoardDeclaration(): string {
  return `【現實材料狀態】目前尚無任何公開發言或行為紀錄，你不可能觀察到任何人的活動、發言量或在場時間，禁止聲稱任何此類觀察（如「他活動頻繁」「他在場時間長」「他有異常的活動模式」「他不太對勁」）。P編號只是代號，沒有大小、遠近、邊緣或中央之分，禁止以編號本身作為理由（如「他位於邊緣」「外圍編號」）。沒有材料時可用的依據只有直覺、隨機嘗試或跟隨他人已提出的建議；直覺就說直覺，不得把直覺包裝成觀察；坦白說「目前沒有特別依據」完全正常。`;
}
