...
function taskInstruction(kind, playerId) {
    switch (kind) {
        ...
        case 'wolf_speech':
            return `【任務】你是 P${playerId}（人狼），請與同伴討論今晚要襲擊誰、協調目標（一句話，30-60字）。直接指名具體目標（P編號）並說理由，如「P3 話多可能是占卜師，先殺他」。守衛保護誰、誰是甚麼職業都是秘密，無從得知，不要討論這類無法觀察的資訊。用「我覺得今晚…」語氣。以你的性格自然表達，但語氣不要過於強烈或浮誇。格式：P${playerId}：「你的發言」；結尾另起一行附加決策旗標[決定:殺P編號]（已決定目標時）或[決定:資訊不足]（尚無法決定時），只可附加其一。`;
    }
}
...
export function buildWolfPreSpeechPrompt(state, playerId) {
    ...
    const parts = [
        `【你的角色資訊】\n${privateLines.join('\n')}`,
        `【今晚狼討論】\n${recent.length > 0 ? recent.join('\n') : '（尚無發言）'}`,
        `【任務】你是 P${playerId}，請寫一句 20-40 字的預發言草稿，與同伴討論今晚要襲擊誰、協調目標（不超過 40 字，中性語氣）。直接指名一個具體目標（P編號）並給理由；守衛保護誰、誰是甚麼職業都是秘密，無從得知，不要討論這類無法觀察的資訊。\n格式：P${playerId}：「你的草稿」`,
