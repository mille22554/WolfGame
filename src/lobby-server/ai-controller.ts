...
export class AiController {
  ...
  private wolfContext(entry: AiEntry): string {
    ...
    return [
      ...
      '',
      '策略思考：',
      '- 誰可能是占卜師、守衛、靈能者？這些角色對狼威脅最大（能識別狼、能保護他人）。',
      '- 刀人優先序：能識別狼的角色 > 能保護他人的角色 > 普通村民。',
      '- 第一天沒有白天發言，但可以從角色配置推測（例如：15 人局通常有占卜師、守衛、靈能者各一）。',
      '- 跟隊友討論「為什麼刀他」，給出策略理由，不是只說「刀他」。',
      '- 推測要標明是推測（「他可能是...」「如果...」），不要把你不知道的事說成事實（「他會被守」「他是占卜師」）。第一天你只知道：狼隊是誰、狂人是誰、總人數。',
    ].filter(Boolean).join('\n');
  }
  ...
  private buildResponsePrompts(entry: AiEntry, publishedSpeech: string): ChatMessage[] {
    const user = [
      ...
      `提醒：直接講你的立場。同意就說「就他」「我跟你」＋理由。不要說「我注意到...」「他剛被提出」。不要提「沒有發言紀錄」「沒人發言」。用你的角色語氣說話。`,
    ].join('\n');
    return [
      { role: 'system', content: `${this.buildSystemPrompt(entry)}\n${this.wolfContext(entry)}` },
      { role: 'user', content: user },
