...
export class AiController {
    ...
    dayContinuation = null;
    dayContinuationEpoch = 0;
    dayRunCounter = 0;
    dayTurnCounter = 0;
    dayResumePromise = null;
    dayResumeEpoch = null;
    dayResumeRunId = null;
    dayTestLlm;
    messageCap;
    ...
    exportDayCheckpoint() {
        ...
        if (gameState.phase !== 'DAY_DISCUSSION' || this.phase !== 'DAY_DISCUSSION')
            return null;
        // Restore barrier 尚未 import 時也可匯出乾淨的初始 continuation；不會啟動 loop。
        const state = this.dayContinuation ?? {
        ...
        if (state.phase !== 'DAY_DISCUSSION')
            return null;
        if (gameState.day !== state.day || this.day !== state.day)
            return null;
    ...
    importDayCheckpoint(snapshot) {
        ...
        for (const clientId of prepared.aiClientIds) {
            ...
            if (entry?.profile && typeof memory === 'string')
                entry.profile.memory = memory;
        }
    ...
    resumeDayDiscussion() {
        if (this.destroyed || !this.game || this.phase !== 'DAY_DISCUSSION')
            return Promise.resolve();
        const gameState = this.game.getNightState();
        if (gameState.phase !== 'DAY_DISCUSSION' || gameState.day !== this.day)
            return Promise.resolve();
        if (this.dayResumePromise &&
    ...
    prepareDayCheckpoint(snapshot) {
        if (!snapshot || typeof snapshot !== 'object' || !this.game)
            return null;
        if (snapshot.schemaVersion !== AI_DAY_CHECKPOINT_SCHEMA_VERSION)
            return null;
        if (snapshot.phase !== 'DAY_DISCUSSION')
            return null;
        if (!Number.isInteger(snapshot.day) || snapshot.day < 1)
            return null;
        ...
        if (!allowedStages.includes(snapshot.stage))
            return null;
        const gameState = this.game.getNightState();
        if (gameState.phase !== 'DAY_DISCUSSION' || gameState.day !== snapshot.day)
            return null;
        ...
        if (!this.sameUniqueClientSet(snapshot.rosterClientIds, currentRoster))
            return null;
        if (!this.sameUniqueClientSet(snapshot.aiClientIds, currentAi))
            return null;
        const aiSet = new Set(snapshot.aiClientIds);
        const rosterSet = new Set(currentRoster);
        if (!Array.isArray(snapshot.strategyDone) || !this.uniqueStringsAreIn(snapshot.strategyDone, aiSet))
            return null;
        if (!Array.isArray(snapshot.draftSlots))
            return null;
        const seenDraftIds = new Set();
        const draftSlots = [];
        for (const raw of snapshot.draftSlots) {
            if (!raw || typeof raw !== 'object' || typeof raw.clientId !== 'string' || !aiSet.has(raw.clientId))
                return null;
            if (seenDraftIds.has(raw.clientId))
                return null;
            ...
            if (hasSpeech !== hasStance)
                return null;
            if (raw.speech !== undefined && !hasSpeech)
                return null;
            if (raw.stance !== undefined && !hasStance)
                return null;
            draftSlots.push({
        ...
        if (snapshot.selectedClientId !== null && !aiSet.has(snapshot.selectedClientId))
            return null;
        if (snapshot.expandedText !== null && typeof snapshot.expandedText !== 'string')
            return null;
        if (snapshot.expandedText !== null && snapshot.selectedClientId === null)
            return null;
        if (snapshot.selectedClientId === null && (snapshot.expandedText !== null || snapshot.published !== null))
            return null;
        let published = null;
        if (snapshot.published !== null) {
            ...
            if (!raw || typeof raw !== 'object')
                return null;
            if (typeof raw.turnId !== 'string' || raw.turnId.length === 0 || typeof raw.clientId !== 'string' || !aiSet.has(raw.clientId))
                return null;
            if (raw.clientId !== snapshot.selectedClientId || snapshot.expandedText === null)
                return null;
            if (raw.status !== 'pending' && raw.status !== 'committed')
                return null;
            if (raw.messageId !== null && (typeof raw.messageId !== 'string' || raw.messageId.length === 0))
                return null;
            if (raw.messageSeq !== null && (!Number.isInteger(raw.messageSeq) || raw.messageSeq < 1))
                return null;
            if (typeof raw.readyValue !== 'boolean')
                return null;
            if (raw.status === 'pending' && (raw.messageId !== null || raw.messageSeq !== null))
                return null;
            published = {
                turnId: raw.turnId,
                clientId: raw.clientId,
        ...
        for (const raw of snapshot.responses) {
            if (!raw || typeof raw !== 'object' || typeof raw.clientId !== 'string' || !aiSet.has(raw.clientId))
                return null;
            if (seenResponseIds.has(raw.clientId))
                return null;
            seenResponseIds.add(raw.clientId);
            if (!published || raw.turnId !== published.turnId || raw.clientId === published.clientId)
                return null;
            if (raw.status !== 'pending' && raw.status !== 'ready' && raw.status !== 'speak' && raw.status !== 'wait')
                return null;
            if (raw.status === 'speak') {
                if (!raw.draft || typeof raw.draft.speech !== 'string' || !raw.draft.speech || typeof raw.draft.stance !== 'string' || !raw.draft.stance)
                    return null;
        ...
        if (responses.length > 0 && (!published || published.status !== 'committed'))
            return null;
        if (!snapshot.profileMemory || typeof snapshot.profileMemory !== 'object' || Array.isArray(snapshot.profileMemory))
            return null;
        const memoryKeys = Object.keys(snapshot.profileMemory);
        if (memoryKeys.length !== aiSet.size || !memoryKeys.every((clientId) => aiSet.has(clientId)))
            return null;
        for (const clientId of aiSet) {
            const memory = snapshot.profileMemory[clientId];
            if (memory !== null && typeof memory !== 'string')
                return null;
            if (memory !== null && !this.entries.get(clientId)?.profile)
                return null;
        }
        if (!Array.isArray(snapshot.knowledge))
            return null;
        const seenKnowledgeIds = new Set();
        for (const raw of snapshot.knowledge) {
            if (!raw || typeof raw !== 'object' || typeof raw.clientId !== 'string' || !aiSet.has(raw.clientId))
                return null;
            if (seenKnowledgeIds.has(raw.clientId))
                return null;
            seenKnowledgeIds.add(raw.clientId);
            if (typeof raw.role !== 'string' || typeof raw.displayName !== 'string')
                return null;
            if (!Array.isArray(raw.partners) || !raw.partners.every((value) => typeof value === 'string'))
                return null;
            if (raw.madman !== null && typeof raw.madman !== 'string')
                return null;
            if (typeof raw.privateInfo !== 'string' || !Array.isArray(raw.recentMessages))
                return null;
            if (!raw.recentMessages.every((message) => message && typeof message.from === 'string' && typeof message.text === 'string'))
                return null;
        }
        if (seenKnowledgeIds.size !== aiSet.size)
            return null;
        if (!this.validBoardSnapshot(snapshot.wolfBoard) || !this.validBoardSnapshot(snapshot.masonBoard))
            return null;
        if (snapshot.selectedClientId !== null && !rosterSet.has(snapshot.selectedClientId))
            return null;
        return {
    ...
    sameUniqueClientSet(value, expected) {
        if (!Array.isArray(value) || !value.every((id) => typeof id === 'string'))
            return false;
        const unique = new Set(value);
        return unique.size === value.length && expected.length === unique.size && expected.every((id) => unique.has(id));
    }
    uniqueStringsAreIn(value, allowed) {
        if (!Array.isArray(value) || !value.every((entry) => typeof entry === 'string'))
            return false;
        const unique = new Set(value);
        return unique.size === value.length && [...unique].every((entry) => allowed.has(entry));
    }
    validBoardSnapshot(value) {
        return Array.isArray(value) && value.every((message) => !!message && typeof message === 'object' && typeof message.from === 'string' && typeof message.text === 'string');
    }
    ...
    handleBroadcast(msg, targetClientIds) {
        ...
        if (m.type === 'PHASE_CHANGED') {
            ...
            if (typeof m.day === 'number')
                this.day = m.day;
            if (this.phase !== 'DAY_DISCUSSION' && this.dayContinuation)
                this.invalidateDayRun();
    ...
    async generateDayStrategies(aiPlayers, state, runId) {
        const done = new Set(state.strategyDone);
        await Promise.all(aiPlayers.map(async (player, index) => {
            if (done.has(player.clientId) || !this.isCurrentDayRun(state, runId))
                return;
            ...
            if (this.dayTestLlm) {
                ...
                const result = await this.llmWithRetry(player.clientId, 'DAY_STRATEGY', state.day, prompts, (parsed) => typeof parsed.strategy === 'string' && parsed.strategy.trim() ? 'ok' : null, 100 + index);
                if (result.value === 'ok' && result.parsed)
                    strategy = result.parsed.strategy.trim();
            }
            ...
            if (!state.strategyDone.includes(player.clientId))
                state.strategyDone.push(player.clientId);
        }));
    }
    /** 依角色生成策略 prompt */
    ...
    async runDayDiscussion(state, runId) {
        ...
        while (this.isCurrentDayRun(state, runId)) {
            ...
            switch (state.stage) {
                ...
                case 'judge': {
                    ...
                    if (!this.isCurrentDayRun(state, runId) || !selected)
                        return;
    ...
    ensureDraftSlots(state, players) {
        const existing = new Set(state.draftSlots.map((slot) => slot.clientId));
        for (const player of players) {
            if (!existing.has(player.clientId))
                state.draftSlots.push({ clientId: player.clientId });
        }
    }
    completedDayDrafts(state, aiPlayers) {
        const valid = new Set(aiPlayers.map((player) => player.clientId));
        return state.draftSlots.filter((slot) => valid.has(slot.clientId) && typeof slot.speech === 'string' && slot.speech.length > 0 && typeof slot.stance === 'string' && slot.stance.length > 0);
    }
    resetDayJudgeState(state) {
    ...
    async generateDayDrafts(players, state, runId) {
        ...
        await Promise.all(players.map(async (player, index) => {
            ...
            if (!slot || typeof slot.speech === 'string' || !this.isCurrentDayRun(state, runId))
                return;
            const entry = this.entries.get(player.clientId);
            if (!entry)
                return;
            ...
            const result = await this.llmWithRetry(player.clientId, 'DAY_SPEECH', state.day, prompts, (parsed) => typeof parsed.speech === 'string' && parsed.speech.trim() && typeof parsed.stance === 'string' ? 'ok' : null, 100 + index);
            if (!this.isCurrentDayRun(state, runId))
                return;
    ...
    async expandDaySpeech(entry, draftSpeech, day) {
        if (this.dayTestLlm) {
            try {
    ...
    publishSelectedDayDraft(state, aiPlayers, runId) {
        if (!this.isCurrentDayRun(state, runId) || !this.game)
            return;
        const selectedId = state.selectedClientId;
        const expandedText = state.expandedText;
        if (!selectedId || expandedText === null || !aiPlayers.some((player) => player.clientId === selectedId))
            return;
        ...
        for (const player of aiPlayers) {
            if (player.clientId === published.clientId || responseIds.has(player.clientId))
                continue;
    ...
    async runDayResponses(state, aiPlayers, runId) {
        if (!this.isCurrentDayRun(state, runId) || !this.game || !state.published || state.published.status !== 'committed')
            return;
        ...
        await Promise.all(aiPlayers.map(async (player, index) => {
            if (!responderIds.has(player.clientId) || !this.isCurrentDayRun(state, runId))
                return;
            ...
            if (response.status === 'ready')
                this.commitDayReady(player.clientId, true, state, runId);
            else if (response.status === 'speak')
                this.commitDayReady(player.clientId, false, state, runId);
        }));
        if (!this.isCurrentDayRun(state, runId))
            return;
    ...
    commitDayReady(clientId, ready, state, runId) {
        if (!this.isCurrentDayRun(state, runId) || !this.game)
            return;
        if (this.game.getDayState().dayReady.get(clientId) === ready)
            return;
        this.game.setDayReady(clientId, ready);
        if (this.isCurrentDayRun(state, runId))
            this.dayReadyMap.set(clientId, ready);
    }
    forceAiDayReady(aiPlayers) {
        if (!this.game)
            return;
        for (const player of aiPlayers) {
            if (this.game.getDayState().dayReady.get(player.clientId) === true)
                continue;
    ...
    async dayRespond(player, publishedSpeech, priority, day) {
        if (this.dayTestLlm) {
            try {
                const result = await this.dayTestLlm.respond(player.clientId, publishedSpeech, day);
                if (result.action === 'ready')
                    return { type: 'ready', strategyUpdate: result.strategyUpdate };
        ...
        const result = await this.llmWithRetry(player.clientId, 'DAY_STANCE', day, prompts, (parsed) => {
            if (parsed.action === 'ready')
                return 'ok';
            if (parsed.action === 'speak' && typeof parsed.speech === 'string' && parsed.speech.trim() && typeof parsed.stance === 'string')
                return 'ok';
