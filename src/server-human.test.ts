...
import { startServer, isTakeoverFiltered, type ServerHandle, type ServerLLM } from './server.js';
...
interface WSMsg {
  type: string;
  lobby?: { phase: string; expectedPlayerCount: number; started: boolean; seats: { playerId: number; name: string; controlledBy: string }[]; playerCount?: number; randomCount?: boolean; engineStatus?: { state: string }; spectators?: { clientId: string; name: string }[]; hostClientId?: string };
...
test('SET_NAME：具名→LOBBY 名單；空名／重名拒收；超長截斷', async () => {
  ...
  try {
    ...
    assert.ok(ok.token);
    const lobby = await waitFor(a, (m) =>
      m.type === 'LOBBY' && (m.lobby!.spectators ?? []).some((s) => s.name === '阿水'));
...
test('改名即時更新名單：座位改名→LOBBY＋之後聊天用新名', async () => {
  ...
  try {
    ...
    const lobby = await waitFor(a, (m) =>
      m.type === 'LOBBY' && m.lobby!.seats[0].name === '新老大');
...
test('觀眾 token 重連：同 token 拿回原名，不另增觀眾', async () => {
  ...
  try {
    ...
    const lobby = await waitFor(c, (m) =>
      m.type === 'LOBBY' && (m.lobby!.spectators ?? []).some((s) => s.name === '漂流者'));
...
test('LEAVE_LOBBY：房主離開→host 轉給最長在場參戰者，開局權限跟著走', async () => {
  ...
  try {
    ...
    send(a, { type: 'LEAVE_LOBBY' });
    await waitFor(a, (m) => m.type === 'LEFT_LOBBY');
    const moved = await waitFor(b, (m) =>
      m.type === 'LOBBY' && m.lobby!.hostClientId === idB && m.lobby!.seats[0].controlledBy === 'empty');
