...
function taskInstruction(kind, playerId) {
    switch (kind) {
        case 'speech':
            return `【任務】你是 P${playerId}，請進行白天發言（一句話，20-60字，短一點沒關係）。圍繞「誰的反應讓你在意」「想聽聽誰的說法」聊，用「我比較在意…」語氣，避免直接定罪。像一般人自然講話，不要套用角色口頭禪或壓力台詞，語氣不要強烈或浮誇。全篇只能使用繁體中文，嚴禁任何簡體字（如杀/发/对/说）。格式：P${playerId}：「你的發言」`;
        case 'vote':
        ...
        case 'wolf_speech':
            return `【任務】你是 P${playerId}（人狼）。這是狼隊內部會議，你和同伴都是人狼，你們在討論今晚襲擊誰，不是在找狼人——不要說「他可能是人狼」。最重要：直接指名一個目標（P編號）並簡述依據（僅當討論紀錄中真有此依據時才可引用；沒有材料時誠實說直覺或隨機即可，不要編造理由）。格式：P${playerId}：「發言」（20-60字，「我覺得今晚…」語氣，自然口語，不要套用口頭禪或壓力台詞）。結尾另起一行 [決定:殺P編號] 或 [決定:資訊不足]。全篇繁體中文，禁止任何英文（tonight/kill/target/maybe）與簡體字。同盟不可襲擊；不得以守衛相關猜測作為理由。`;
    }
}
...
export function buildPreSpeechPrompt(state, playerId) {
    ...
    const parts = [
        ...
        `【任務】你是 P${playerId}，請寫一句 20-40 字的預發言草稿（不超過 40 字）。\n這是候選草稿，稍後可能被選中展開。用一般人的自然語氣寫，不要刻意扮演角色口吻、不要浮誇；人格設定只作為你的思考傾向參考（懷疑誰、在意什麼），不要求模仿其說話風格。全篇只能使用繁體中文，嚴禁任何簡體字（如杀/发/对/说）。圍繞當前局勢，提出一個值得討論的點；沒有材料時可談直覺或對局勢的疑問，不要編造對他人的觀察。注意：只要草稿中明確說出想投誰（即使理由只是直覺），就是已決定；只有連點名誰都拿不定時，才算資訊不足。\n格式：P${playerId}：「你的草稿」`,
        `【決策旗標】草稿結尾另起一行附加你的投票準備狀態（中控內部判讀用，不會公開）：已決定投某人（說出具體目標即算已決定，即使理由只是直覺）→[決定:投P編號]；已決定棄票→[決定:棄票]；連提名誰都拿不定→[決定:資訊不足]。只可附加其一。`,
    ];
...
function wolfLegalIds(state, playerId) {
...
function buildWolfRulesBlock(legalTargets) {
...
export function buildWolfPreSpeechPrompt(state, playerId) {
    ...
    const parts = [
        ...
        dayBlock,
        recent.length === 0
            ? `【輸出】提案目標（例：「我覺得殺P0」，P0 是佔位符，換成上面列表中的合法編號）。不要複述規則、不要談論怎麼說話。格式：一句 5-30 字發言；另起一行 [決定:殺P編號] 或 [決定:資訊不足]（指名目標→殺P編號，未指名→資訊不足）。全篇繁體中文，禁止任何英文（tonight/kill/target/maybe）與簡體字。`
            : `【輸出】回應同伴：同意→「同意，殺P0」「跟票P0」（P0 是佔位符，編號必須抄自上面的討論紀錄）；目標不同→點出分歧（例：「P0說P1，我選P2」，皆為佔位符）。上面已有人提的編號你要贊成→附和（例：「就P0」），不要再用「我覺得」開新提案。你同意時，編號必須抄自上面的討論紀錄；上面沒出現過的編號不能用「同意」引出，要當新提案講。措辭必須與上面每句都不同。格式：一句 5-30 字發言；另起一行 [決定:殺P編號] 或 [決定:資訊不足]（指名目標→殺P編號，未指名→資訊不足）。全篇繁體中文，禁止任何英文（tonight/kill/target/maybe）與簡體字。`,
    ];
    let prompt = parts.filter((s) => s !== '').join('\n\n');
