...
export class AiController {
  ...
  private async runDayDiscussion(): Promise<void> {
    ...
    while (this.phase === 'DAY_DISCUSSION' && !this.destroyed) {
      ...
      for (const p of aiPlayers) {
        if (this.destroyed) return;
        if (p.clientId === selected.player.clientId) continue;
        const entry = this.entries.get(p.clientId);
        if (!entry) continue;
  ...
  private wolfContext(entry: AiEntry): string {
    ...
    return [
      ...
      '',
      '邏輯紅線（違反 = 策略無效，嚴禁）：',
      '- 你今晚刀的人，明天是死人。不會發言、不會自證、不會被投票、不會出現在白天討論。你绝对不能把白天策略的對象設為你今晚刀的人。',
      '- 刀人目標 ≠ 白天討論對象。你刀了 A，明天白天討論的是 B、C、D（還活著的人）。你的白天策略要針對「明天還活著的玩家」，不是針對你今晚的刀人目標。',
      '- 白天策略的方向：你在村民面前表演。你要做的是「讓某個存活村民顯得可疑」或「製造讓村民互相猜疑的氛圍」或「自保（如果被質疑怎麼回應）」。不是「把焦點引向已死之人」。',
      '- 白天策略必須是具體動作：「我明天先發言質疑健太」「我明天裝白，被問就答...」「我明天安靜，觀察誰在護佐雪」。不要寫模糊觀察（「看看大家反應」「呼籲冷靜」）——那不是策略。',
      '- 提到任何玩家時用名字，不用「他」「她」——私頻裡代詞會搞混。',
    ].filter(Boolean).join('\n');
  }
  ...
  private buildDraftPrompts(entry: AiEntry): ChatMessage[] {
    const user = [
      ...
      `稱呼：用名字稱呼隊友（如「良子你明天負責攻擊」），不要用「我跟你」——多人討論時「你」會搞混。`,
      ``,
      `回覆格式（JSON）：`,
  ...
  private buildResponsePrompts(entry: AiEntry, publishedSpeech: string): ChatMessage[] {
    const user = [
      ...
      `要求：同意就簡短，但要用目標的名字（「就刀小晴」「我投小晴」），不要說「就他」——「他」會跟隊友名字搞混。不用重述隊友已經講過的理由。要講就講新的角度（不同的風險、明天的計畫、你的保留意見）。不要重複白板上已有的內容。`,
      `提醒：直接講你的立場。不同意就說「我投另一个人」＋你的理由。不要盲目跟隊友。不要說「我注意到...」「他剛被提出」。用名字稱呼隊友，不要用沒有名字的「我跟你」。用你的角色語氣說話。`,
    ].join('\n');
    return [
      { role: 'system', content: `${this.buildSystemPrompt(entry)}\n${this.wolfContext(entry)}` },
      { role: 'user', content: user },
