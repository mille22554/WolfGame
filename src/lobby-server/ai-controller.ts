...
export class AiController {
  ...
  private wolfContext(entry: AiEntry): string {
    ...
    return [
      ...
      '- 第一天資訊少，直覺猜是正常的。但你的猜要落在具體角色上（「他可能是占卜師」），並說清楚為什麼先刀這個角色對狼最有利。不要說「他怪怪的」「感覺不對」——那不是策略。',
      '- 想一步：如果刀錯瞭，明天能學到什麼？（例如：「如果太助明天沒死，就是被守了，我們就知道守衛保誰」）。',
      '- 推測要標明是推測（「他可能是...」「如果...」），不要把你不知道的事說成事實。',
  ...
  private buildDraftPrompts(entry: AiEntry): ChatMessage[] {
    const user = [
      ...
      `要求：speech 要落在具體角色＋策略（「他可能是占卜師，先處理掉比較穩」）。直覺可以，但結論要是角色判斷，不是「他怪怪的」。推測說「可能是」，不要說成事實。`,
