...
狼會議是一場**持續的對話**（不是離散的「出稿→評選」回合）。所有存活狼在狼白板（WOLF_CHAT）上討論，judge 每輪選出一篇代表發言，其他狼讀完表態（同意→投票；不同意→接著聊），直到全狼同意才進投票。沒有「舊草稿」概念——每輪分歧的狼是接著聊（新訊息建立在白板之前的對話上），不是重新出稿。

狀態機（loop，直到全狼同意）：

| 狀態 | 說明 | 過渡 |
|---|---|---|
| **0** 首句發言 | 所有存活狼各發一句（WOLF_CHAT），提出刀人目標＋理由（所有人思考） | → 0.5 |
| **0.5** judge 選一句 | judge 從最新一輪的發言裡選出策略品質最高的一篇（全盲評分，不告知作者） | → 1 |
| **1** 其他人讀＋表態 | 其他狼讀那篇，各自判斷：資訊夠、同意 → toggle「準備投票」ON；資訊不夠／不同意／還有事要討論 → 接著聊 | 同意 → 1_1；不同意 → 2 |
| **1_1** 投票 | 所有狼都 toggle ON → 各狼投 WOLF_KILL：沒平票 → 最高票者出局（確定刀人目標）；平票 → 回 0（所有狼「準備投票」重置 OFF） | 沒平票 → 完成 step 3；平票 → 0 |
| **2** 接著聊 | 分歧的狼發新訊息（連續對話，新訊息建立在白板之前的對話上，不是重新出稿） | 都發完 → 2_1；還沒發完、有人發言 → 2_2 |
| **2_1** 都發完 | 分歧的狼都發完新訊息 | → 0（judge 再從最新一輪選一篇） |
| **2_2** 還沒發完 | 還沒發完、有人發言 | → 0.5（judge 從目前的發言裡先選一篇） |

**收斂**：loop 重複（0 → 0.5 → 1 → 2 → 0 ...）直到所有狼都 toggle「準備投票」ON（全票同意）→ 進 1_1 投票 → 確定刀人目標。
...
- 人狼刀：狼會議收斂（全狼 toggle「準備投票」ON）後，取收斂的刀人目標；投票平票→回討論重來（見上方狼會議流程）
...
AI 狼依 §12.3 的狀態機驅動（非 SpeechScheduler 管線，是持續對話）：

| 狀態 | AI 狼的動作 |
|---|---|
| **0** 首句發言 | 每隻 AI 狼 LLM 生成一句（刀人目標＋理由，≤50 字，符合 persona）→ 發 `WOLF_CHAT` |
| **0.5** judge 選一句 | server 端 judge LLM 全盲評分最新一輪發言、選最高分 → broadcast `WOLF_SPEECH_SELECTED`（非 AI 狼動作） |
| **1** 其他人讀＋表態 | 每隻「非代表狼」的 AI 狼 LLM 判斷：接受代表狼的目標，或反對？接受 → toggle `TOGGLE_WOLF_READY` ON；反對 → 發新訊息（接著聊） |
| **1_1** 投票 | 每隻 AI 狼 LLM 選刀人目標 → 提交 `WOLF_KILL`（不可選自己/狂人） |
| **2** 接著聊 | 分歧的 AI 狼 LLM 生成新訊息（建立在白板之前的對話上，符合 persona）→ 發 `WOLF_CHAT` |

- **收斂**：loop 重複（0→0.5→1→2→0...）直到全狼 toggle ready → 進投票
- **安全上限（測試用）**：白板累計 100 則 `WOLF_MESSAGE` 未收斂 → 停止並報告（不自動收斂）
- **LLM 失敗**：重試取得回覆（見 §13.2）；最終失敗 → 該狼跳過本次動作（不阻塞，由安全上限兜底）
