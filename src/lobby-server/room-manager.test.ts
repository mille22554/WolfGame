...
test('joinRoom：滿房但 asSpectator=true → 以觀戰身分加入', () => {
  ...
  const room = rm.getRoom(c.code)!;
  room.maxPlayers = 6;
  for (let i = 2; i <= 6; i++) rm.joinRoom(c.code, `P${i}`, `c${i}`, 'ipx');
...
test('joinRoom：遊戲已開始 → 一律以觀戰身分加入', () => {
  ...
  const r = rm.joinRoom(c.code, 'B', 'c2', 'ip2');
  assert.equal(r.ok, true);
  if (r.ok) assert.equal(r.isSpectator, true);
});
...
test(`joinRoom：總人數達上限 ${MAX_MEMBERS_PER_ROOM} → FULL_CAP`, () => {
  ...
  assert.ok(c.ok);
  const room = rm.getRoom(c.code)!;
  for (let i = 2; i <= 15; i++) rm.joinRoom(c.code, `P${i}`, `c${i}`, 'ipx');
  for (let i = 1; i <= 5; i++) rm.joinRoom(c.code, `S${i}`, `s${i}`, 'ipx', true);
...
test('setSetting：maxPlayers 超範圍或小於目前玩家數 → INVALID', () => {
  ...
  const c = rm.createRoom('A', 'c1', 'ip1');
  assert.ok(c.ok);
  for (let i = 2; i <= 7; i++) rm.joinRoom(c.code, `P${i}`, `c${i}`, 'ipx');
...
test('setMode：觀戰→參戰（有空位）成功；席位滿 → NO_SLOT', () => {
  ...
  assert.equal(ok.ok, true);
  if (ok.ok) assert.equal(ok.room.getMember('s1')?.isSpectator, false);
  const back = rm.setMode('s1', 'spectate');
  assert.equal(back.ok, true);
  if (back.ok) assert.equal(back.room.getMember('s1')?.isSpectator, true);
});
