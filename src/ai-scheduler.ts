...
export function collectWolfViolations(raw: string, st: GameState, pid: number): { kind: string; hit: string }[] {
  ...
  const push = (kind: string, hit: string): void => {
    if (hit && !out.some((e) => e.kind === kind && e.hit === hit)) out.push({ kind, hit });
  };
  if (isEnglishHeavy(raw)) push('lang', '英文超標');
  const engWord = findEnglishWord(raw);
  if (engWord) push('lang', `英文短詞(${engWord})`);
  const hitSeed = findGroundingViolation(raw);
  if (hitSeed) push('grounding', hitSeed);
...
export function collectExpandViolations(raw: string, firstNight: boolean): { kind: 'grounding' | 'lang'; hit: string }[] {
  const out: { kind: 'grounding' | 'lang'; hit: string }[] = [];
  const seed = findGroundingViolation(raw);
  if (seed) out.push({ kind: 'grounding', hit: seed });
  if (firstNight) {
    const fab = findFirstNightFabrication(raw);
    if (fab && fab !== seed) out.push({ kind: 'grounding', hit: fab });
  }
  const eng = findEnglishWord(raw);
  if (eng) out.push({ kind: 'lang', hit: `英文短詞(${eng})` });
  return out;
}
...
// ============================================
// 外部積累賬本 precedents.jsonl（跨局前科；append-only，無上限，只增不減）
// ============================================
...
export function findRecentPrecedentsByKind(meeting: string, phase: string, file?: string): PrecedentEntry[] {
  let all: PrecedentEntry[] = [];
  try {
    all = readPrecedents(file);
  } catch { return []; }
  ...
  const out: PrecedentEntry[] = [];
  const pushKind = (kind: string): void => {
    const hit = same.find((e) => e.kind === kind);
    if (hit) out.push(hit);
  };
  pushKind('grounding');
  if (simp) out.push(simp);
  if (eng) out.push(eng);
  pushKind('target');
  pushKind('format');
  return out;
...
export class SpeechScheduler implements AIScheduler {
  ...
  private candidateIds(state: GameState): number[] {
    ...
    const readySet = isWolf ? [] as number[] : state.voteReady;
  ...
  private async runPreSpeechCandidate(token: number, pid: number): Promise<Omit<Draft, 'slot'> | null> {
    ...
    if (isWolf) {
      ...
      try {
        ...
        if (recents.length > 0) basePrompt = `${basePrompt}\n\n${recents.map(buildCrossGameNote).join('\n')}`;
      } catch { /* 無賬本照舊 */ }
    }
    ...
    } catch { return this.abstainPre(pid, isWolf); }
    if (!rawRaw) return this.abstainPre(pid, isWolf);
    if (isWolf) {
      // 全量記賬：一稿命中 N 種記 N 筆（同 text；處置仍一次棄權）
      const pending: { kind: string; hit: string; text: string }[] = [];
      const raw = normalizeTraditional(rawRaw);
      if (simplifiedRejection(rawRaw)) pending.push({ kind: 'lang', hit: '簡體混入', text: raw });
      for (const v of collectWolfViolations(raw, st, pid)) pending.push({ kind: v.kind, hit: v.hit, text: raw });
      if (pending.length > 0) {
    ...
    if (!text) return this.abstainPre(pid, isWolf);
    return { playerId: pid, text, decision };
  }

  /** 單次棄權兜底：狼記 uncertain 不播出；白天回 null（不計安全閥） */
  private abstainPre(pid: number, isWolf: boolean): null {
    if (isWolf) this.updateDecision(pid, { status: 'uncertain' });
    return null;
  }
  ...
  private async fetchExpandText(expandPrompt: string, isWolfMode: boolean, pid: number, st: GameState): Promise<string> {
    ...
    const raw = normalizeTraditional(rawRaw);
    if (!isWolfMode) return raw;
    // 全量記賬：一稿命中 N 種記 N 筆（處置仍一次退草稿）
    const pending: { kind: 'grounding' | 'lang'; hit: string; text: string }[] = [];
    if (simplifiedRejection(rawRaw)) pending.push({ kind: 'lang', hit: '簡體混入', text: raw });
    for (const v of collectExpandViolations(raw, firstNight)) pending.push({ kind: v.kind, hit: v.hit, text: raw });
