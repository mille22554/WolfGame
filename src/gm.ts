...
import {
  initGame, loadState, saveState, getStateSnapshot,
  startDay, processNight, addMessage, processVotes, revealRoles,
  GMNightActions, GMVote,
} from './game-state.js';
...
switch (command) {
  ...
  case 'night': {
    const state = loadState();
    const actions: GMNightActions = JSON.parse(args[1] || '{}');
  ...
  case 'vote': {
    const state = loadState();
    const votes: GMVote[] = JSON.parse(args[1] || '[]');
