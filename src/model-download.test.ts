...
): Promise<{ server: http.Server; port: number; hits: () => number }> {
  let hits = 0;
...
test('downloadModelFile：停滯（連上無資料）→ 下載停滯超時 abort + tmp 清理', async (t) => {
  ...
  t.after(() => {
    if (prev === undefined) delete process.env.DOWNLOAD_STALL_TIMEOUT_MS;
    else process.env.DOWNLOAD_STALL_TIMEOUT_MS = prev;
  });
  // 連上但永不回應任何 bytes
  const { server, port } = await withServer((_req, _res) => { /* hang */ });
  const dir = mkTempDir();
  try {
    await assert.rejects(
      () => downloadModelFile(`http://localhost:${port}/stall.gguf`, dir),
      /下載停滯超時/,
    );
