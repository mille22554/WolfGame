...
export class AiController {
  ...
  private buildSystemPrompt(entry: AiEntry): string {
    return [
      ...
      '',
      '**【P6｜自然口語（硬規則）】** 你說的是人話，不是電報。每句話都要是完整、通順、口語化的句子——像正常人聊天那樣把話講完整。禁止：單詞式質問（「少講？」「刀她？」）、破碎斷句、突兀插入語、把不相干的短句硬拼在一起。要把話講完整、因果通順。',
      '**【P13｜精簡（硬規則）】** 不要鋪陳、不要裝飾。接受分派就是一句「好，我輔助你」等級的直接確認，接著只講實質動作。禁止「你的分派我接」「打法沒毛病」這類只有氣氛沒有資訊的開場——每個字都要承載資訊，砍掉「解釋為什麼要這樣打」的場面話。',
      '**【P14｜動作必須具體（硬規則）】** 每個動作都要能直接執行，寫得出具體內容才能寫。「質疑對方」＝寫成「叫他報出昨晚查了誰」；寫不出具體內容的動作描述（「追問他」「盯緊他」「看風向」）＝空話，禁止。',
      '**【P9｜禁 meta 確認句（硬規則）】** 禁止「回應對方話語本身」的冗詞開頭——例如「你那句我接」「我收到了」「同意你這說法」——這些廢話不承載任何資訊。直接進入內容。每個字都要有資訊量。',
      '**【P11｜語言純度（硬規則）】** 全繁體中文。禁止任何日文漢字（「占い師」「発言」等）與簡體字（「当场」「为什么」等）混入——「占卜師」「發言」「當場」「為什麼」。若分不清某個字是繁/簡/日，改用完全不同的詞或重寫整個句子，不許讓可疑字進 speech。（本規則優先於 persona 範例——persona 範例若出現混雜用字，以本規則為準）',
      '**【P12｜長發言排版（硬規則）】** 發言超過約 50 字時，用換行分段，每段一個重點，總長控制在 3 段以內。段序：① 判斷或立場（結論先行）→ ② 你的動作／接受或拒絕 → ③ （若有）給隊友的提醒。短發言（≤50 字）不分段，保持一氣呵成。',
      '',
      '## 場上證據邊界（硬規則，優先於一切）',
  ...
  private wolfContext(entry: AiEntry): string {
    ...
    return [
      ...
      '',
      '刀人優先序：占卜師 > 守衛/靈能者 > 村民。',
      '第一天無白天發言，角色判斷都是猜。選誰都可以，沒有理由就不用解釋。禁止用「氣場」「感覺不對」等形容詞充當理由——沒有觀察就是沒有理由。',
      '',
      '**【P4｜說謊是狼的本職】** 你是狼，說謊就是你的武器。假跳占卜師時，查驗結果隨便你編——村民不知道那是假的，直到有人能反駁。評估欺騙戰術時不要用「我們拿不出證據/沒依據」當反對理由——編一個就好。',
      '**【P5｜演技決定成敗】** 假跳/對跳能不能騙過村民，取決於演得像不像、邏輯自洽不自洽——場上沒人能確認誰是真占卜師，村民二選一靠誰講得合理。拆穿是機率不是必然。評估假跳戰術要算「這謊怎麼圓才能騙過村民」，不是「反正會被拆穿」。',
      '**【P10｜對跳資訊結構＋天數感知（硬規則）】**',
      '1. 真占卜師反跳＝他一定有真實查驗結果，他一定報得出自己查了誰。「他會答不出來」「他會接不住」「他會卡住」這些概念全部不存在——他永遠有話講。',
      '2. 雙方報完結果後就是「雙CO」成局：村民看到兩個占卜師，必須選邊。勝負在「兩個結果哪個更可信、誰演得像、誰前後一致」。',
      '3. 假跳真正目的＝釣真占現身、晚上刀他——他現身就是目標達成，白天誰被信是次要的。',
      '4. 追問武器的天數限制：',
      '   - 第 1 天雙CO：沒有「過程/依據」可以追——真占第 1 夜查誰是隨機或猜測，他答「我隨機挑的」「憑直覺」就過了，「你為什麼查他」全是空拳。第一天能較量的只有態度、先後、語氣與一致性——演技戰。',
      '   - 第 2 天起：「過程」武器才解鎖——追問「你昨天懷疑誰、今天查的是誰」「兩夜結果怎麼連」才有攻擊力。',
      '**【P16｜禁幻想破綻（硬規則）】** 真占反跳後不會改口、不會自亂陣腳、不會前後矛盾——他會一直堅守同一個查驗結果，重複幾遍都一樣。禁止假設「他會改口」「他會露出破綻」「他會慌」「他會講錯」——第一天這些事一件都不會發生，等於沒有。質疑他的唯一目的是把場面推向「兩個結果對決、村民二選一」；他報得出結果是正常反應，不是破綻。要贏靠你演得像、態度穩、把場面變成雙CO對決，不靠他犯錯。同理同夥也絕不會改口——她的破綻也不會自己出現。',
      '**【P15｜對跳期白天投票鎖定（硬規則）】** 假跳/對跳成局期間，你白天的票鎖定假跳宣稱的那個人（例：同夥宣稱「某人是人狼」→ 你白天表態與投票都鎖那個人）。禁止「看真占點誰就跟誰」：真占的查驗結果可能剛好點到狼隊（同夥或你自己），跟票＝把劍架在自己人脖子上，極度不利。除非狼會議重新協議，否則白天票不離開假跳宣稱目標。（注意：這跟「今晚刀誰」是兩回事——夜晚刀人照會議決議走。）',
      '',
      '**禁止（違反＝策略無效）：**',
      ...
      '- 對跳（被動回應）：真占卜師先 CO 後，狼跟跳占卜師製造「雙CO」混亂。成敗看演技與圓謊能力（第 1 天對跳＝演技戰，沒有過程可追）。',
  ...
  private buildDraftPrompts(entry: AiEntry): ChatMessage[] {
    ...
    const user = [
      ...
      ``,
      `任務：說一句話——① 你刀誰（會議已有共識目標時可省略）② 你明天白天做什麼（針對存活玩家）③ 你預期這個動作讓會議怎麼發展（潛伏時可省略）。長發言超過約 50 字須換行分段（≤3 段，P12）。`,
      `狼每晚必須刀人。「資訊不足」＝你還在考慮，最終必須選。`,
      ``,
      `出稿前先想清楚四件事，再寫 speech：`,
      `① 刀的目標服務哪個軸（夜間消耗/白天存活＋引導）？為什麼是他（沒有理由就不用寫）？`,
      `② 明天的白天動作選哪個：潛伏（三個都活著優先、最安全）／引導投票（把票導向某人、好處大但暴露風險高）／製造假資訊（發言讓村民互猜）／假跳或對跳占卜師（編查驗結果、製造雙CO混亂）？為什麼？記住：第 1 天雙CO 時沒有過程可追、只能打演技戰（P10）。`,
      `③ 你的行動必須跟所選模式一致：`,
      ...
      `- 假跳/對跳：寫清楚編什麼查驗結果、誰來跳、怎麼圓謊、拆穿風險多高（P4/P5/P10）。`,
  ...
  private buildResponsePrompts(entry: AiEntry, publishedSpeech: string): ChatMessage[] {
    ...
    const user = [
      ...
      `- 戰術比較：用狼戰術字典的招跟目前路線比較——含假跳/對跳占卜師（編查驗結果、製造雙CO混亂）在內，有沒有更划算的選擇？值不值得為它調整刀人目標？若你認為該假跳/對跳，說清楚編什麼結果、誰來跳、怎麼圓謊、拆穿風險多高。`,
      `- P10/P16 提醒：第 1 天雙CO，追問「為什麼查他」是空拳（P10），真占不會改口不會自亂陣腳（P16）——質疑他的唯一目的＝把場面變成「兩個結果對決、村民二選一」。你白天能做的具體動作只有：叫他報結果、語氣比穩、帶票鎖假跳宣稱目標（P15）。`,
      `評估完沒問題 → 純附和（vote）完全沒問題。評估發現問題 → speak 指出問題＋替代方案。`,
      ``,
      `**【P1｜回答義務】** 隊友的發言如果是在回應你之前的問題或分派你任務——先直接回應（接受/拒絕＋理由），再談其他。禁止繞開問題。接受分派就用一句話確認（P13），不必複述她的計畫。`,
      `**【P2｜假設情境規則】** 隊友發言若是假設性戰術討論，假設視為已成立——直接面對「真發生時該怎麼做」。禁止用「現在還沒發生」反駁假設前提。`,
      `**【P8｜配合分工出口（品質關卡版）】**`,
      `- 計畫需要你配合（幫腔、壓場、接話、圓謊、引導投票）時——不要只丟空票，用 speak 講清楚你的配合動作。`,
      `- 沒有實質內容要交代時，純 vote 就是正確且完整的回應，不要硬擠一句。`,
      `- 論點品質關卡（硬規則）：每一句你想說的話，先自問兩關——(1) 村民聽到的第一反應會不會是「這廢話/這我看得見」？(2) 對方或旁觀村民有沒有輕而易舉的反駁？任何一關過不了，這句話就不准說。`,
      `- 只覆蓋高機率分支（硬規則）：預演只針對「真占反跳、報出自己的查驗結果」這個高機率局面。禁止鑽低機率連鎖假設。`,
      ``,
      `你的立場（三選一）：`,
      `1. 同意投票（我同意目前的刀人目標，我準備投票）→ {"action": "vote", "target": "今晚刀人目標的名字"}`,
      `2. 我要補充（我有新角度或要改變立場）→ {"action": "speak", "speech": "完整通順口語、角色語氣、全繁體中文、無meta確認句；超過50字須換行分段（≤3段）", "stance": "投XXX"}`,
