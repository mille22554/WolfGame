...
test('下載 URL 變體＋目錄名純函數', () => {
    // cpu 維持現行資產名
    assert.equal(llamaServerDownloadUrl('b10361', 'cpu'), 'https://github.com/ggml-org/llama.cpp/releases/download/b10361/llama-b10361-bin-win-cpu-x64.zip');
    assert.equal(llamaServerDownloadUrl('b10361'), 'https://github.com/ggml-org/llama.cpp/releases/download/b10361/llama-b10361-bin-win-cpu-x64.zip');
    // vulkan 變體
    assert.equal(llamaServerDownloadUrl('b10361', 'vulkan'), 'https://github.com/ggml-org/llama.cpp/releases/download/b10361/llama-b10361-bin-win-vulkan-x64.zip');
...
test('GPU 錯誤 regex：命中 Vulkan 無裝置／OOM／驅動不足，排除一般 crash', () => {
    ...
    for (const s of gpuCases)
        assert.equal(isGpuCrashError(s), true, `應命中：${s}`);
    const generalCases = [
        'fake crash',
        'boom-xyz-diagnostic',
    ...
    for (const s of generalCases)
        assert.equal(isGpuCrashError(s), false, `不應命中：${s}`);
});
...
test('ensureLlamaServer 變體分目錄：cpu／vulkan 各就其位', async () => {
    const mkFetch = (marker) => {
        const zip = new AdmZip();
        zip.addFile('llama-server.exe', Buffer.from(marker));
        const buf = zip.toBuffer();
        return (async () => new Response(buf, { status: 200, headers: { 'content-length': String(buf.length) } }));
    };
...
`);
    ...
    const result = await start({
        ...
        onStatus: (status, info) => { events.push({ status, info }); },
        createManager: (o) => {
            if (o.binPath === vulkanFake)
                gpuLayersSeen.push(o.gpuLayers ?? 0);
...
test('回退狀態機：一般 crash 不走回退（直接丟原錯）', async () => {
    ...
    try {
        await assert.rejects(startLlamaServerWithFallback({
            cpuBinPath: cpuFake,
            vulkanBinPath: vulkanFake,
            modelPath: 'x.gguf',
            port,
...
`);
    ...
    try {
        await mgr.start();
        const args = JSON.parse(fs.readFileSync(outFile, 'utf-8'));
        ...
        if (prev === undefined)
            delete process.env.FAKE_ARGS_OUT;
        else
            process.env.FAKE_ARGS_OUT = prev;
        await mgr.stop();
    }
    ...
    try {
        await mgr2.start();
        const args = JSON.parse(fs.readFileSync(outFile2, 'utf-8'));
