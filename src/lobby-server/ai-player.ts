...
export interface AiPlayerInfo {
  id: number;
  name: string;         // romaji（內部識別用）
  displayName: string;  // 中文名（顯示/發言用）
  role: string;
...
function buildSystemPrompt(profile: CharacterProfile, role: string, extraContext: string): string {
  ...
  return [
    ...
    `- 直接說內容。禁止「我先講...」「讓我說一下...」等前言。禁止「不是...而是...」「與其說...不如說...」等對立修正句型——直接說你的判斷。`,
    `- 理由必須基於 prompt 中實際給出的資訊。禁止使用不適用於當前情境的抽象策略語言（例如：候選列表已排除狼隊友，不要說「避免誤傷友方」）。如果沒有具體資訊就直說「沒有足夠資訊，我直覺選他」。`,
    `- 發言聚焦在「刀誰＋為什麼」。不要附加模糊的未來行動計畫（如「明天順勢追問」）。要嘛不說，要嘛說具體的。`,
