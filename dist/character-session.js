...
function stripPressureBlock(personaPrompt) {
    if (!personaPrompt.includes('壓力台詞'))
...
function pickWolfExampleId(legalIds) {
    if (legalIds.length === 0)
        return 1;
    return legalIds[Math.floor(Math.random() * legalIds.length)];
}
/** 首夜正向選單任務段（版本A措辭照抄；旗標另立獨立段） */
function buildWolfFirstNightTask(playerId, exampleId, legalTargets) {
...
function buildWolfFirstNightFlag() {
