...
function mixedNightState(): GameState {
...
function wolves(s: GameState): number[] {
  return s.players.filter((p) => p.role === Role.WEREWOLF && p.alive).map((p) => p.id);
}

function nonWolfVictim(s: GameState, exclude: number): number {
...
test('getNightActors 含全部存活狼（2 狼局 → seer + 2 狼，day1 無 guard）', () => {
  ...
  for (const id of w) assert.ok(actors.includes(id), `狼 P${id} 應在 night gate`);
...
test('DISCONNECT（NIGHT_COLLECTING gate 內未完成）→ 產生 DISPATCH_LLM effect', () => {
  ...
  const humanInGate = gate.required.find((id) =>
    s.players.find((p) => p.id === id)!.controlledBy === 'human');
  ...
  assert.ok(r.effects.some((e) => e.type === 'DISPATCH_LLM' && (e as { playerId: number }).playerId === humanInGate));
});
