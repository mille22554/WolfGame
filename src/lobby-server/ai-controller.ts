...
export class AiController {
  ...
  private wolfContext(entry: AiEntry): string {
    ...
    return [
      ...
      '- 第一天你沒有白天發言可參考，對每個人的判斷都是猜。承認這是猜，但說清楚你猜的邏輯（例如：「沒有資訊，先刀他，明天看村子反應」「他位置靠後，可能是神職」）。不要假裝你有依據。',
      '- 想一步：如果刀錯瞭，明天能學到什麼？（例如：「如果太助明天沒死，就是被守了，我們就知道守衛保誰」）。',
      '- 推測要標明是推測（「他可能是...」「如果...」），不要把你不知道的事說成事實。第一天你只知道：狼隊是誰、狂人是誰、總人數。',
      '- 你不需要跟隊友選同一個人。如果你有不同的判斷，說出來並給理由。全票一致不是目標，有討論才是。',
    ].filter(Boolean).join('\n');
  }
  ...
  private buildDraftPrompts(entry: AiEntry): ChatMessage[] {
    const user = [
      ...
      `要求：speech 必須包含策略理由（為什麼刀他）。例如：「他可能是占卜師，先處理掉比較穩」「他太安靜了，可能是守衛在觀察」。不要只說「我直覺選他」——直覺不是策略。推測要說「可能是」，不要把你不知道的事說成事實。`,
      `提醒：你在跟隊友即時對話。不要說「我注意到...」「他剛被提出」。不要提「沒有發言紀錄」「沒人發言」——第一天夜裡本來就沒有。資訊少就從角色配置推測（「15 人局他可能是守衛」）。用你的角色語氣說話。`,
      ``,
      `回覆格式（JSON）：`,
  ...
  private buildResponsePrompts(entry: AiEntry, publishedSpeech: string): ChatMessage[] {
    const user = [
      ...
      `要求：如果你選擇「我要講」，speech 必須包含策略理由（為什麼支持或反對這個目標）。同意時可以簡短（「就他，他可能是占卜師」），但要帶理由。推測要說「可能是」，不要說成事實。`,
      `提醒：直接講你的立場。同意就說「就他」＋理由；不同意就說「我投另一个人」＋理由。不要盲目跟隊友。不要說「我注意到...」「他剛被提出」。不要提「沒有發言紀錄」「沒人發言」。用你的角色語氣說話。`,
    ].join('\n');
    return [
      { role: 'system', content: `${this.buildSystemPrompt(entry)}\n${this.wolfContext(entry)}` },
      { role: 'user', content: user },
