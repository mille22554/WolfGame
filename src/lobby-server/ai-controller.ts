...
export class AiController {
  ...
  private masonContext(entry: AiEntry): string {
    ...
    return [
      ...
      '',
      '**禁止：** 質疑沒發言的人、無依據攻擊；用代詞指代玩家、提到自己時用「我」不用自己名字；speech 帶編號/機制用語（白板/會議/私頻）；「我跟你」「同意夥伴」類讓人看出同盟的呼應。',
      '**正確：** 質疑引用實際發言；具體動作（拋什麼話題、觀察誰）；用名字。',
      '',
      '## 戰術字典',
  ...
  private buildMasonResponsePrompts(entry: AiEntry, publishedSpeech: string): ChatMessage[] {
    ...
    const user = [
      ...
      `⚠️ 白天動作只針對存活玩家。用名字，不用代詞。此頻道只有你們兩個，不引用白板外發言。`,
      `⚠️ 會議已有共識的白天安排時，speech 不再重宣布（「就這麼辦」「我跟你」這類）——只想重申既定安排／沒有新戰術 → 直接 action="vote"。speech 只能用來提新的分工（誰拋話題、誰觀察、被質疑時誰救、怎麼配合）。`,
