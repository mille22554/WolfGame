...
export type ClientToServerMessage =
  ...
  | { type: 'JOIN_ROOM'; code: string; nickname: string; asSpectator?: boolean }
...
export type ServerToClientMessage =
  ...
  | { type: 'SPECTATOR_JOINED'; code: string; isHost: boolean; players: MemberInfo[]; started: boolean; maxPlayers: number; randomCount: boolean }
  | { type: 'ROOM_FULL' }
  | { type: 'MESSAGE'; from: string; text: string; ts: number }
  | { type: 'PLAYER_JOINED'; nickname: string }
  | { type: 'PLAYER_LEFT'; nickname: string }
  | { type: 'HOST_CHANGED'; newHost: string }
  | { type: 'SETTING_CHANGED'; maxPlayers: number; randomCount: boolean }
