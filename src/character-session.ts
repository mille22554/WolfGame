...
function taskInstruction(kind: PromptKind, playerId: number): string {
  switch (kind) {
    ...
    case 'wolf_speech':
      return `【任務】你是 P${playerId}（人狼），請與同伴討論今晚要襲擊誰、協調目標（一句話，30-60字）。直接指名具體目標（P編號）並說理由，如「P3 話多可能是占卜師，先殺他」（僅當討論紀錄中真有此觀察時才可用行為理由；沒有材料時誠實說直覺或隨機即可，不要編造理由）。你的同盟列在【你的角色資訊】，襲擊同盟是規則上不可能的行為，絕對不要考慮。守衛保護誰、誰是甚麼職業都是秘密，無從得知：不得聲稱知道，也不得以「守衛可能會保護P編號」這類預測作為選擇或排除目標的理由。請使用繁體中文。用「我覺得今晚…」語氣。以你的性格自然表達，但語氣不要過於強烈或浮誇。格式：P${playerId}：「你的發言」；結尾另起一行附加決策旗標[決定:殺P編號]（已決定目標時）或[決定:資訊不足]（尚無法決定時），只可附加其一。`;
  }
}
...
function emptyBoardDeclaration(): string {
  return `【現實材料狀態】目前尚無任何公開發言或行為紀錄，你不可能觀察到任何人的活動、發言量或在場時間，禁止聲稱任何此類觀察（如「他活動頻繁」「他在場時間長」）。P編號只是代號，沒有大小、遠近、邊緣或中央之分，禁止以編號本身作為理由（如「他位於邊緣」「外圍編號」）。沒有材料時可用的依據只有直覺、隨機嘗試或跟隨他人已提出的建議；坦白說「目前沒有特別依據」完全正常。`;
}
...
export function buildWolfPreSpeechPrompt(state: GameState, playerId: number): string {
  ...
  const parts: string[] = [
    ...
    `【今晚狼討論】\n${recent.length > 0 ? recent.join('\n') : '（尚無發言）'}`,
    hasNoPublicBehaviorRecord(state) ? emptyBoardDeclaration() : '',
    `【任務】你是 P${playerId}，請寫一句 20-40 字的預發言草稿，與同伴討論今晚要襲擊誰、協調目標（不超過 40 字，中性語氣）。直接指名一個具體目標（P編號）並給理由；沒有材料時誠實說直覺或隨機即可，不要編造理由。今晚可襲擊的存活玩家只有：${wolfValidTargets(state, playerId)}（你的同盟不在其中，襲擊同盟是規則上不可能的行為，不要考慮）。守衛保護誰、誰是甚麼職業都是秘密，無從得知：不得聲稱知道，也不得以「守衛可能會保護P編號」這類預測作為選擇或排除目標的理由。請使用繁體中文。\n格式：P${playerId}：「你的草稿」`,
