...
import { GameEngine, type GameCallbacks, type GamePlayer } from './game.js';

interface Captured {
  sendTo: { clientId: string; msg: any }[];
  broadcast: { msg: any; targets?: string[] }[];
  wolfSubphaseChanges: { subphase: string; round: number }[];
}

/** 假 callbacks：記錄所有 sendTo / broadcast / onWolfSubphaseChange（mock AI 控制器掛鉤） */
function makeHarness() {
  const cap: Captured = { sendTo: [], broadcast: [], wolfSubphaseChanges: [] };
...
function roleOf(game: GameEngine, role: Role): GamePlayer[] {
  return game.getPlayers().filter((p) => p.role === role);
}

function broadcastsWithType(cap: Captured, type: string): any[] {
...
test('狼會議：3 狼 2:1 → 明確多數收斂（無 WOLF_VOTE_SPLIT）', () => {
  ...
  try {
    ...
    for (const m of masons) game.handleToggleMasonEndTurn(m.clientId);
    ...
    for (const w of wolves) game.handleToggleWolfReady(w.clientId);
...
test('狼會議：1:1:1 平票 → 回討論（round+1、ready 重置）→ 再投 2:1 收斂', () => {
  ...
  try {
    ...
    assert.deepEqual(
      Object.fromEntries(Object.entries(splits[0].votes).map(([k, v]) => [game.getPlayers().find((p) => p.clientId === k)?.nickname, v])),
      { [t1.nickname]: 1, [t2.nickname]: 1, [t3.nickname]: 1 },
    );
...
test('狼會議：白板累計 100 則未收斂 → 停止並報告（不自動收斂、後續訊息／toggle 被忽略）', () => {
  ...
  try {
    ...
    // 灌 100 則訊息（未收斂）
    for (let i = 0; i < 100; i++) game.handleWolfChat(wolves[0].clientId, `msg ${i}`);
    const state = game.getNightState();
