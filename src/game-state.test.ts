...
import {
  createGameState, transition, getNightActors, saveState, loadState, stripSpeechPrefix,
} from './game-state.js';
...
function joinedState(count = 9, humans: number[] = []): GameState {
  const s = createGameState(count, humans);
  joinAll(s, count);
  return s;
}

function startedState(count = 9, humans: number[] = []): GameState {
...
function toDiscussion(s: GameState): void {
  completeNight(s);
...
function toVotingAllAI(s: GameState): void {
  toDiscussion(s);
...
test('NIGHT_COLLECTING：gate 完成 → NIGHT_RESOLVING + ENQUEUE RESOLVE_NIGHT', () => {
  ...
  assert.ok(last.effects.some((e) => e.type === 'ENQUEUE' && (e as { event: { type: string } }).event.type === 'RESOLVE_NIGHT'));
});
...
test('NIGHT_RESOLVING：狼襲擊目標是同盟 → 過濾並回退隨機非狼（禁止狼殺狼）', () => {
  ...
  // 其餘角色照常行動
  for (const pid of getNightActors(s)) {
    if (wolves.some((w) => w.id === pid)) continue;
...
test('DAY_DISCUSSION_OPEN：voteReady 增減；全員 ready → 直進 DAY_VOTING_COLLECTING', () => {
  ...
  for (const h of humans) transition(s, { type: 'HUMAN_READY_VOTE', playerId: h });
