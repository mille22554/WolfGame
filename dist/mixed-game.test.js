...
function assertFlat(s) {
    for (const p of s.players.filter((x) => x.alive)) {
        for (const kind of ['speech', 'vote', 'night', 'wolf_speech']) {
...
function driveDiscussion(engine, humanActions) {
    const s = engine.getState();
    // 真人發言 → 跳過 → 準備投票
    ...
    // AI 發言一則（版本相符）
    const speaker = aliveIds(engine.getState()).find((id) => engine.getState().players.find((p) => p.id === id).controlledBy === 'ai');
...
//# sourceMappingURL=mixed-game.test.js.map