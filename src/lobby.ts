...
import type { EngineStatus, LobbySnapshot, Phase } from './types.js';
...
export class LobbyManager {
  ...
  private tokens = new Map<string, number>(); // token → playerId（離座/斷線保留，重連拿回）
  ...
  private resetSeats(n: number): void {
    this.seats = [];
  ...
  setPlayerCount(n: number): { droppedTokens: string[] } {
    assertCount(n);
  ...
  resolveCount(): number {
    if (!this._randomCount) return this._playerCount;
    let maxSeat = 0;
    for (const s of this.seats) {
      if (s.controlledBy === 'human' && s.playerId > maxSeat) maxSeat = s.playerId;
    }
  ...
  leave(playerId: number): void {
    const seat = this.seats[playerId - 1];
    if (!seat || seat.controlledBy !== 'human') return;
  ...
  leaveLobbySeat(playerId: number, token?: string): void {
    ...
    this.leave(playerId);
    if (seatToken !== undefined) this.limbo.delete(seatToken);
    if (token !== undefined) this.limbo.delete(token);
  }

  /** 大廳內同 token 重連：斷線座位（human+disconnected）直接拿回；limbo（已離座）座位空才恢復 */
  reclaim(token: string): { playerId: number } | undefined {
    ...
    if (!seat || seat.controlledBy !== 'empty') return undefined;
    seat.controlledBy = 'human';
  ...
  markDisconnected(playerId: number): void {
    const seat = this.seats[playerId - 1];
    if (seat && seat.controlledBy === 'human') seat.disconnected = true;
  }
  ...
  seatName(playerId: number): string | undefined {
    const seat = this.seats[playerId - 1];
    if (!seat || seat.controlledBy !== 'human' || seat.name === '') return undefined;
    return seat.name;
  }
  ...
  addChat(from: string, text: string): LobbyChatEntry {
    ...
    if (clean.length === 0) throw new Error('訊息不可為空');
    if (clean.length > CHAT_MAXLEN) throw new Error(`訊息過長（上限 ${CHAT_MAXLEN} 字）`);
  ...
  fillAiSeats(): void {
    for (const s of this.seats) {
      if (s.controlledBy === 'empty') s.controlledBy = 'ai';
    }
  }
  ...
  seatsForStart(): { playerId: number; name: string; controlledBy: 'ai' | 'human' }[] {
  ...
  snapshot(): LobbySnapshot {
    return {
      phase: 'SETUP_WAITING_JOIN' as Phase,
