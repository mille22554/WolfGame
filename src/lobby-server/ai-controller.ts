...
export class AiController {
  ...
  private wolfContext(entry: AiEntry): string {
    ...
    return [
      ...
      '',
      '策略思考：',
      '- 誰可能是占い師、守衛、霊能者？這些角色對狼威脅最大（能識別狼、能保護他人）。',
      '- 刀人優先序：能識別狼的角色 > 能保護他人的角色 > 普通村民。',
      '- 第一天沒有白天發言，但可以從局勢推測（例如：15 人局通常有占い師、守衛、霊能者各一）。',
  ...
  private buildDraftPrompts(entry: AiEntry): ChatMessage[] {
    const user = [
      ...
      `要求：speech 必須包含策略理由（為什麼刀他）。例如：「他可能是占い師，先處理掉比較穩」「他太安靜了，可能是守衛在觀察」。不要只說「我直覺選他」——直覺不是策略。`,
  ...
  private buildResponsePrompts(entry: AiEntry, publishedSpeech: string): ChatMessage[] {
    const user = [
      ...
      `要求：如果你選擇「我要講」，speech 必須包含策略理由（為什麼支持或反對這個目標）。同意時可以簡短（「就他，他可能是占い師」），但要帶理由。`,
