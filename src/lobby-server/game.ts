...
export class GameEngine {
  ...
  setDayReady(clientId: string, ready: boolean): void {
    ...
    if ((this.state.dayReady.get(clientId) ?? false) === nextReady) return;
  ...
  reconcileDayReady(): void {
    if (this.state.phase !== 'DAY_DISCUSSION') return;
    const alivePlayers = this.getAlivePlayers();
    if (alivePlayers.length === 0 || alivePlayers.some((p) => this.state.dayReady.get(p.clientId) !== true)) return;
    this.transitionTo('DAY_VOTING');
  }
  ...
  publishMasonSpeech(speakerClientId: string, text: string, round: number): void {
    ...
    this.callbacks.broadcast(
      { type: 'MASON_SPEECH_SELECTED', round, from: player.nickname, text: clean },
      targets,
    );
  }
  ...
  getDayState(): { dayReady: Map<string, boolean>; dayMessages: DayMessage[] } {
    return {
  ...
  restoreState(snapshot: Record<string, any>): void {
    ...
    this.state.players = playerSnapshots.map((p: any) => {
    ...
    this.state.deathHistory = Array.isArray(snapshot.deathHistory)
      ? (snapshot.deathHistory.map((entry: any) => ({ ...entry })) as DeathRecord[])
      : [];
    ...
    this.state.wolfBoard = Array.isArray(snapshot.wolfBoard)
      ? (snapshot.wolfBoard.map((entry: any) => ({ ...entry })) as { from: string; text: string }[])
      : [];
  ...
  private transitionTo(phase: GamePhase, options: { preserveDayState?: boolean } = {}): void {
    ...
    if (phase === 'DAY_DISCUSSION') {
      ...
      if (this.state.phase === 'DAY_DISCUSSION') this.broadcastDayReadyStatus();
      return;
    }
  ...
  private broadcastDayReadyStatus(): void {
    const alivePlayers = this.getAlivePlayers();
  ...
  private normalizeDayReady(value: unknown): Map<string, boolean> {
    ...
    const dayReady = new Map<string, boolean>();
  ...
  private nextDayMessageIdentity(): { day: number; seq: number; id: string } {
    const day = this.state.day;
    let seq = ++this.dayMessageSeq;
    let id = this.makeDayMessageId(day, seq);
    while (
      this.state.dayMessages.some((message) => message.day === day && message.seq === seq) ||
  ...
  private normalizeDayMessages(value: unknown, day: number): DayMessage[] {
    if (!Array.isArray(value)) return [];

    const usedIds = new Set<string>();
    const usedSeqByDay = new Set<string>();
