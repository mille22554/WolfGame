...
export class SpeechScheduler implements AIScheduler {
  ...
  private aliveAIIds(state: GameState): number[] {
  ...
  private async runPipeline(): Promise<void> {
    ...
    try {
      ...
      if (startState.phase !== 'DAY_DISCUSSION_OPEN') return;
      ...
      if (cur1.boardVersion !== startVersion) return;   // 作廢
      if (drafts.length === 0) return;                  // 全部失敗
      ...
      if (token === this.pipelineToken) this.pipelineActive = false;
    }
  }

  /** 存活 AI ≤ 2：跳過管線，直接隨機選一展開 */
  private async runDirect(token: number, ids: number[]): Promise<void> {
  ...
  private async collectPreSpeeches(
    token: number,
    state: GameState,
    ids: number[],
  ): Promise<{ slot: number; playerId: number; text: string }[]> {
    const results: { slot: number; playerId: number; text: string }[] = [];
      ...
      const settled = await Promise.all(chunk.map(async (pid) => {
        ...
        for (let attempt = 0; attempt < 2; attempt++) {
          try {
            ...
            if (text) return { playerId: pid, text };
          } catch { /* 重試 1 次 */ }
        }
        return null;
      }));
      for (const r of settled) {
        if (r) results.push({ slot: 0, playerId: r.playerId, text: r.text });
      }
    }
  ...
  private async judge(
    token: number,
    state: GameState,
    drafts: { slot: number; playerId: number; text: string }[],
    ...
    if (token !== this.pipelineToken) return new Map();
  ...
  private async broadcastAfterCd(
    token: number,
    playerId: number,
    text: string,
...
function buildExpandPromptFallback(state: GameState, playerId: number): string {
