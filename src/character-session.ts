...
export function buildPreSpeechPrompt(state: GameState, playerId: number): string {
  ...
  const parts: string[] = [
    ...
    `【任務】你是 P${playerId}，請寫一句 20-40 字的預發言草稿（不超過 40 字）。\n這是候選草稿，稍後可能被選中展開。用一般人的自然語氣寫，不要刻意扮演角色口吻、不要浮誇；人格設定只作為你的思考傾向參考（懷疑誰、在意什麼），不要求模仿其說話風格。圍繞當前局勢，提出一個值得討論的點；沒有材料時可談直覺或對局勢的疑問，不要編造對他人的觀察。注意：只要草稿中明確說出想投誰（即使理由只是直覺），就是已決定；只有連點名誰都拿不定時，才算資訊不足。\n格式：P${playerId}：「你的草稿」`,
