...
export interface LobbyServerOptions {
  ...
  publicDir?: string;       // default path.join(getResourceRoot(), 'ubuntu-web')
  inactivityTimeoutMs?: number;
  sweepIntervalMs?: number;
  startSweep?: boolean;     // default true; tests may set false
}
...
function serveStatic(req: http.IncomingMessage, res: http.ServerResponse, publicDir: string): void {
...
export async function createLobbyServer(opts: LobbyServerOptions = {}): Promise<LobbyServerHandle> {
  ...
  if (opts.startSweep !== false) roomManager.startSweep();
  ...
  const broadcastToRoom = (code: string, msg: ServerToClientMessage, exceptClientId?: string): void => {
  ...
  const onMessage = (clientId: string, ws: WebSocket, data: unknown): void => {
    ...
    switch (msg.type) {
      ...
      case 'JOIN_ROOM': {
        ...
        if (r.ok) {
          ...
          if (r.isSpectator) send(ws, { type: 'SPECTATOR_JOINED', ...payload });
          else send(ws, { type: 'ROOM_JOINED', ...payload });
  ...
  const onDisconnect = (clientId: string): void => {
    const rec = clients.get(clientId);
    if (!rec) return;
    ...
    if (r.left) broadcastToRoom(code, { type: 'PLAYER_LEFT', nickname: r.left.nickname }, clientId);
    if (r.wasHost && r.newHost) broadcastToRoom(code, { type: 'HOST_CHANGED', newHost: r.newHost.nickname });
  };
