...
export function resolveNightActions(gameState) {
    ...
    if (guard && canGuardAct(guard, currentDay)) {
        // Find guard's action
        const guardAction = gameState.nightActions.find(a => a.actorId === guard.id && a.type === NightActionType.GUARD_PROTECT);
        if (guardAction) {
    ...
    if (seer) {
        const seerAction = gameState.nightActions.find(a => a.actorId === seer.id && a.type === NightActionType.SEER_CHECK);
        if (seerAction) {
 ...
 */
export function getMediumInfoAtDawn(gameState) {
    ...
    const previousDayDeaths = gameState.deathHistory.filter(d => d.day === gameState.day - 1 && d.cause === 'vote');
    if (previousDayDeaths.length === 0)
        return null;
 ...
 */
export function formatNightResult(result) {
 ...
 */
export function formatSeerResult(result) {
 ...
 */
export function formatMediumResult(info) {
    if (info) {
