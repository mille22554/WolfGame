...
export class GameEngine {
    ...
    actionTypeFor(p) {
        ...
        if (p.role === Role.SEER)
            return 'SEER_CHECK';
        if (p.role === Role.GUARD && this.state.day > 1)
            return 'GUARD_PROTECT';
        return null;
    }
    /** 活躍夜間玩家 = 存活人狼 + 存活占い師 + 存活守衛（Day2 起） */
    getNightActivePlayers() {
    ...
    resolveNight() {
        ...
        if (guardAction && guard) {
            ...
            if (guardedTargetId === guard.clientId) {
                ...
                if (others.length > 0)
                    guardedTargetId = others[Math.floor(Math.random() * others.length)].clientId;
            }
        }
        // 2) 人狼：多數決（平票 → 先提交者）；目標 == 護衛目標 → 平安夜
        const wolfActions = this.state.nightActions.filter((a) => a.type === 'WOLF_KILL');
        const wolfCounts = new Map();
        for (const a of wolfActions)
            wolfCounts.set(a.targetClientId, (wolfCounts.get(a.targetClientId) ?? 0) + 1);
