    ...
    try {
        const ready = new Promise((resolve, reject) => {
            worker.on('message', (msg) => {
                if (msg.type === 'READY')
                    resolve();
                else if (msg.type === 'LOG' && msg.level === 'error')
...
test('worker：多個 JOB 全部完成（序列化）', async () => {
    ...
    try {
        ...
        for (const id of ids)
            assert.ok(done.get(id).trim().length > 0);
    }
    finally {
        await worker.terminate().catch(() => undefined);
    }
...
test('dispatcher：start → generate/requestVote/requestNightAction → shutdown', async () => {
    ...
    const night = await withTimeout(d.requestNightAction(2, '請選擇今晚行動的目標'), 10000, 'night');
...
test('dispatcher 崩潰：maxRetries=0 時 pending reject；重啟後重試成功', async () => {
    ...
    try {
        ...
        d.onCrash(new Error('test crash'));
