...
import { pickRandom, findPlayerById } from './utils.js';
import { AIPlayer } from './ai.js';
/**
 * Conduct voting phase
 * All alive players vote for someone to eliminate
 */
export function conductVoting(gameState, humanVoteTargetId) {
 ...
 */
function selectVoteTarget(voter, alivePlayers, gameState) {
 ...
 */
export function formatVotingResultPublic(result) {
 ...
 */
export function formatVotingResultForMedium(result) {
 ...
 */
export function formatDayStart(gameState, nightResult) {
 ...
 */
export function formatDiscussionPrompt(gameState) {
    if (gameState.day === 1) {
        ...
        if (!rec)
            rec = dh.find(d => d.cause === 'wolf_kill' && d.day === gameState.day - 1);
        if (rec)
            victimId = rec.playerId;
        else {
            const rev = [...dh].reverse().find(d => d.cause === 'wolf_kill');
            if (rev)
                victimId = rev.playerId;
        }
        if (victimId === undefined) {
            ...
            if (dead.length > 0) {
                if (dh.length > 0)
                    victimId = dh[dh.length - 1].playerId;
                else
                    victimId = dead[0].id;
            }
        }
