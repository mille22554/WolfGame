...
export function llamaServerDownloadUrl(release: string, variant: LlamaServerVariant = 'cpu'): string {
...
export function llamaServerDirName(release: string, variant: LlamaServerVariant = 'cpu'): string {
  return `llama-${release}-${variant}`;
}

/** 清掉同 release 的非保留變體目錄（手動單變體用；auto 模式需雙包並存，不呼叫）。回傳刪除的目錄。 */
export function pruneOtherVariantDirs(binDir: string, release: string, keep: LlamaServerVariant): string[] {
  const removed: string[] = [];
  for (const v of ['cpu', 'vulkan'] as const) {
    if (v === keep) continue;
...
export function migrateLegacyLlamaDir(binDir: string, release: string): void {
  ...
  try {
    if (!fs.existsSync(path.join(cpuDir, 'llama-server.exe'))) {
      ...
      if (fs.existsSync(cpuDir)) fs.rmSync(cpuDir, { recursive: true, force: true });
...
export function gpuLayersForVram(vramMB?: number, integrated?: boolean): number {
  if (integrated) return 0;
  if (vramMB === undefined || !Number.isFinite(vramMB)) return REDUCED_GPU_LAYERS;
  const gb = vramMB / 1024;
  if (gb >= 6) return FULL_GPU_LAYERS;
  if (gb >= 4) return REDUCED_GPU_LAYERS;
  return 0;
}

/** 自動降層：99 等高層→ 20；20 以下→ 0（改走 CPU，不再重試同 binary） */
export function reducedGpuLayers(layers: number): number {
  return layers > REDUCED_GPU_LAYERS ? REDUCED_GPU_LAYERS : 0;
}

/** 顯存偵測：只吃手動覆寫（env LLAMA_VRAM_MB，MB）；wmic／AdapterRAM 不準，不做 */
export function detectVramMB(env: NodeJS.ProcessEnv = process.env): number | undefined {
  const v = Number(env.LLAMA_VRAM_MB);
  return Number.isFinite(v) && v > 0 ? v : undefined;
}

/** 預設層數：env LLAMA_GPU_LAYERS 強制覆寫 ＞ 顯存分級（未知→保守 20）；內顯（LLAMA_INTEGRATED_GPU=1）→ 0 */
export function defaultGpuLayers(env: NodeJS.ProcessEnv = process.env): number {
...
const GPU_CRASH_PATTERNS: RegExp[] = [
  /no\s+vulkan/i,
...
export function isGpuCrashError(logTail: string): boolean {
...
export function readBackendPreference(dataDir?: string): BackendPreference {
  try {
    const raw = fs.readFileSync(path.join(dataDir ?? getDataDir(), BACKEND_CONFIG_FILE), 'utf-8');
    const v = (JSON.parse(raw) as { backend?: unknown }).backend;
...
export function writeBackendPreference(pref: BackendPreference, dataDir?: string): void {
...
export function effectiveBackendPreference(opts: {
  ...
  const o = opts.option;
  if (o === 'cpu' || o === 'gpu' || o === 'auto') return o;
  const s = opts.stored;
  if (s === 'cpu' || s === 'gpu') return s;
  const e = opts.env?.LLAMA_BACKEND;
  if (e === 'cpu' || e === 'gpu' || e === 'auto') return e;
  return 'auto';
}
...
export async function ensureLlamaServer(options: LlamaServerDownloadOptions = {}): Promise<string> {
  ...
  const variant: LlamaServerVariant = options.variant ?? 'cpu';
  ...
  if (fs.existsSync(variantDownload)) {
    if (options.pruneOtherVariants) pruneOtherVariantDirs(binDir, release, variant);
  ...
  if (options.pruneOtherVariants) pruneOtherVariantDirs(binDir, release, variant);
  if (fs.existsSync(variantDownload)) return variantDownload;
 ...
 */
export async function ensureLlamaServerPair(options: LlamaServerDownloadOptions = {}): Promise<{
  cpuPath: string;
  ...
  let vulkanPath: string | null = null;
  try {
...
export class LlamaServerManager {
  private readonly options: Required<Omit<LlamaServerManagerOptions, 'onStatus'>> &
  ...
  private spawnChild(port: number): ChildProcess {
  ...
  private killSync(child: ChildProcess): void {
    try {
      if (!isDead(child)) child.kill();
    } catch {
      /* ignore */
    }
  }
  ...
  private async portOccupiedByOther(port: number): Promise<boolean> {
  ...
  async probeHealth(port: number): Promise<boolean> {
    try {
      const ctrl = new AbortController();
      const timer = setTimeout(() => ctrl.abort(), 2000);
      try {
...
function isDead(child: ChildProcess): boolean {
  return child.exitCode !== null || child.signalCode !== null;
}

// ============================================
// 自動回退狀態機（auto／手動 gpu 共用；只用碟上包，全程無二次下載）
// 順序：Vulkan(全層) →（GPU 錯誤才）同包降層一次 →（仍 GPU 錯誤才，auto 限定）碟上 CPU 包。
// 非 GPU 錯誤直接 throw（一般 crash，不走回退）。
// ============================================
...
export async function startLlamaServerWithFallback(
  options: LlamaFallbackStartOptions,
): Promise<LlamaFallbackStartResult> {
  const {
    cpuBinPath, vulkanBinPath, modelPath,
    allowCpuFallback = true, createManager,
    onStatus, gpuLayers: gpuLayersOpt,
    ...rest
  } = options;
  const mk = createManager ?? ((o: LlamaServerManagerOptions) => new LlamaServerManager(o));
  const baseOpts = { modelPath, ...rest, onStatus } as Omit<LlamaServerManagerOptions, 'binPath'>;

  const startOne = async (
    binPath: string, layers: number, extraStatus?: (s: 'starting' | 'ready' | 'crashed' | 'stopped', i?: string) => void,
  ): Promise<{ manager: LlamaServerManager; port: number; reused: boolean; crashedTail: string }> => {
    let crashedTail = '';
    const mgr = mk({
      ...baseOpts,
      binPath,
    ...
    try {
      ...
      if (!crashedTail) crashedTail = `${mgr.getLogTail()}\n${msg}`;
  ...
  const vulkan = vulkanBinPath as string;
  ...
  try {
    ...
    const tail = (e as { crashedTail: string }).crashedTail ?? '';
    const err = (e as { err: unknown }).err;
    if (!isGpuCrashError(tail)) throw err; // 一般 crash：不回退，直接丟原錯（已含一般 crash 診斷）
    ...
    if (reduced > 0) {
      ...
      try {
        ...
        const tail2 = (e2 as { crashedTail: string }).crashedTail ?? '';
        const err2 = (e2 as { err: unknown }).err;
        if (!isGpuCrashError(tail2)) throw err2; // 降層後非 GPU 錯：一般 crash
        if (!allowCpuFallback) {
