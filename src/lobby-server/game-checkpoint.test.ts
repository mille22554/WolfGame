...
interface Captured {
  ...
  phaseSnapshots: { phase: string; day: number; ready: Map<string, boolean>; messageCount: number }[];
}

function makeHarness(playerCount = 3): { game: GameEngine; cap: Captured; players: { clientId: string; nickname: string }[] } {
  const cap: Captured = { broadcast: [], phaseSnapshots: [] };
...
function countBroadcasts(cap: Captured, type: string): number {
...
test('dayMessages 50 則上限：第 51 則後仍只保留最近 50 則', () => {
  const { game, players } = makeHarness();
  try {
    for (let i = 1; i <= 51; i++) game.sendDayMessage(players[0].clientId, `message-${i}`);
...
test('oversized dayMessages restore 仍截為 50 則，下一則 seq 不碰撞', () => {
  ...
  const restored = makeHarness();
  try {
    const snapshot = source.game.saveState() as any;
...
test('wolfTargetId 會 round-trip；舊 snapshot 或不存在的 target 安全降為 null', () => {
  ...
  try {
    ...
    (source.game as any).state.wolfTargetId = targetId;
    ...
    restored.game.restoreState({ ...snapshot, wolfTargetId: 'missing-player' } as any);
    assert.equal(restored.game.getNightState().wolfTargetId, null);

    const legacySnapshot = { ...snapshot } as any;
...
test('正常進入新一天會清空 dayReady 與當日 day board', () => {
  ...
  try {
    ...
    game.setDayReady(players[0].clientId, true);
    (game as any).state.day = 2;
    (game as any).transitionTo('DAY_DISCUSSION');
