...
export function buildPrivateKnowledge(player:Player,gameState:GameState):PrivateKnowledge{
  ...
  if(player.role===Role.SEER&&player.seerChecks) k.mySeerChecks=[...player.seerChecks];
  if(player.role===Role.GUARD&&player.guardProtects) k.myGuardProtects=[...player.guardProtects];
  if(player.role===Role.MASON) k.masonPartnerId=player.masonPartnerId;
  if(player.role===Role.WEREWOLF) k.wolfAllyIds=getAliveWerewolves(gameState.players).map(w=>w.id);
  return k;
}
const ACCUSATORY_KEYWORDS=['懷疑','可疑','投','票','帶風向','怪','狼','嫌疑','出去','壓'] as const;
const KNIFE_NEAR_PATTERNS=[/不刀/,/刀\s*P/,/刀他/,/砍/,/挑軟/,/為何不刀/,/為什麼不刀/];
export function isAccusatoryContext(msg:string,idx:number,len:number):boolean{
  const ctx=msg.slice(Math.max(0,idx-10),Math.min(msg.length,idx+len+10));
  for(const pat of KNIFE_NEAR_PATTERNS) if(pat.test(ctx)) return false;
  for(const kw of ACCUSATORY_KEYWORDS) if(ctx.includes(kw)) return true;
  if(/為什麼是|為何是|怎麼是|為什麼\s*P|為何\s*P/.test(ctx)) return true;
  return false;
}
export function parseAccusatoryIds(msg:string,alivePlayers:Player[]):number[]{
  const re=/P(\d+)/g; const ids:number[]=[]; let m:RegExpExecArray|null;
  while((m=re.exec(msg))!==null){const id=parseInt(m[1],10); if(!alivePlayers.some(p=>p.id===id)) continue; if(ids.includes(id)) continue; if(!isAccusatoryContext(msg,m.index,m[0].length)) continue; ids.push(id);}
  return ids;
}
export function cleanTarget(id:number|string):string{
  const raw=String(id).trim(); const stripped=raw.replace(/^P+/g,''); const num=stripped.replace(/[^0-9]/g,'');
...
export function hasStatedOn(targetId:number,XId:number,messages:DiscussionEntry[]):boolean{
  for(const m of messages){
    ...
    while((match=re.exec(msg))!==null){
      ...
      if(isAccusatoryContext(msg,idx,len)) return true;
    }
  }
  return false;
}
export function getVoteTargetOf(id:number,messages:DiscussionEntry[]):number|null{
  for(let i=messages.length-1;i>=0;i--){ const m=messages[i]; if(m.playerId!==id) continue; const msg=m.message;
    ...
    while((match=re.exec(msg))!==null){ const pid=parseInt(match[1],10); const ctx=msg.slice(Math.max(0,match.index-16),Math.min(msg.length,match.index+match[0].length+8)); if(/想投|投|票|站邊|掛票/.test(ctx)){ if(/不刀|刀\s*P|刀他|砍|挑軟/.test(ctx)) continue; lastFound=pid; } }
    if(lastFound!==null) return lastFound;
    const pats:RegExp[]=[/想投\s*[：:，, ]*\s*P(\d+)\b/,/投\s*[：:，, ]*\s*P(\d+)\b/,/票\s*[^P]{0,6}P(\d+)\b/,/站邊[^P]{0,8}P(\d+)\b/,/掛票[^P]{0,8}P(\d+)\b/];
    for(const pat of pats){ const mt=msg.match(pat); if(mt) return parseInt(mt[1],10); }
  }
  return null;
}
export interface ReasoningContext{ suspectRanking:number[]; confidence:number; keyEvidence:string; }
export function computeReasoningContext(player:Player,gameState:GameState):ReasoningContext{
  const alivePlayers=getAlivePlayers(gameState.players);
  ...
  const wolfAllyIds:number[]=isWolf?getAliveWerewolves(gameState.players).map(w=>w.id).filter(id=>id!==player.id):[];
  const masonPartner=player.masonPartnerId;
  const seerWolfTargets=new Set<number>((player.seerChecks||[]).filter(c=>c.result===SeerResult.WEREWOLF).map(c=>c.targetId));
  const scores=new Map<number,number>(); const evidenceMap=new Map<number,string>();
  for(const c of candidates){
    let score=10; const mentions=mentionCountFor(c.id); score+=mentions*8;
    const lm=lastMentionIdx(c.id), ls=lastSpeakIdx(c.id); const unresponded=lm!==-1&&lm>ls; if(unresponded) score+=15;
    const sc=speakCountFor(c.id); if(sc===0) score+=10;
    let seerEvidence=false; if(seerWolfTargets.has(c.id)&&alivePlayers.some(p=>p.id===c.id)){ score+=100; seerEvidence=true; }
    if(wolfAllyIds.includes(c.id)) score-=50;
    if(masonPartner!==undefined&&masonPartner===c.id) score-=30;
    score+=gameState.votes.filter(v=>v.targetId===c.id).length; score+=randomInt(0,3);
    scores.set(c.id,score);
    let ev='';
    if(seerEvidence){ const phs=[`我查驗到 P${c.id} 為狼`,`我昨晚查驗了 P${c.id}，結果是狼`,`P${c.id} 是狼，我的查驗結果不會錯`,`我查驗過 P${c.id} 了，他是狼`]; ev=pickRandom(phs);
    } else if(unresponded&&mentions>=2&&sc<=1) ev=`P${c.id} 被多人點名但一直沒有回應`;
    else if(unresponded&&mentions>=2) ev=`P${c.id} 被多人點名但回應避重就輕`;
    else if(unresponded) ev=`P${c.id} 被點名後沒有回應，迴避態度明顯`;
    else if(mentions>=3) ev=`P${c.id} 一直是話題中心，但本人回應避重就輕`;
    else if(mentions>=2) ev=`P${c.id} 被多人點名但回應不足`;
    else if(mentions>=1) ev=`P${c.id} 是討論焦點之一，但說法含糊`;
    else if(sc===0) ev=`P${c.id} 今天完全沉默，缺乏資訊反而可疑`;
    else ev=`P${c.id} 的發言與投票節奏讓我在意`;
    const recentDeath=gameState.deathHistory.length>0?gameState.deathHistory[gameState.deathHistory.length-1]:undefined;
    if(recentDeath&&mentions===0&&sc===0) ev+='，且在有死者的情況下保持沉默';
    evidenceMap.set(c.id,ev);
  }
  const sorted=Array.from(scores.entries()).sort((a,b)=>b[1]-a[1]); const suspectRanking=sorted.map(([id])=>id);
  let confidence=30; const topId=suspectRanking[0]; const topScore=scores.get(topId)??0; const secondScore=suspectRanking.length>1?(scores.get(suspectRanking[1])??0):0; const gap=topScore-secondScore;
  const hasSeerWolfTop=topId!==undefined&&seerWolfTargets.has(topId);
  if(hasSeerWolfTop) confidence=92;
  else if(topScore>=40&&gap>=12) confidence=Math.min(90,72+gap);
  else if(topScore>=30&&gap>=7) confidence=Math.min(75,55+gap*1.5);
  else if(todayLog.length>=5&&topScore>=22) confidence=45+Math.min(15,gap*2)+randomInt(0,5);
  else { confidence=20+Math.min(15,topScore*0.5)+randomInt(0,5); if(todayLog.length<3) confidence=Math.min(confidence,35); }
...
export class AIPlayer{
  constructor(public player:Player,private gameState:GameState){}
  private buildReasoningContext():ReasoningContext{ return computeReasoningContext(this.player,this.gameState); }
  decideNightAction():NightAction|null{
    switch(this.player.role){ case Role.WEREWOLF: return this.decideWolfKill(); case Role.SEER: return this.decideSeerCheck(); case Role.GUARD: return this.decideGuardProtect(); default: return null; }
  }
  private decideWolfKill():NightAction{
    const alivePlayers=getAlivePlayers(this.gameState.players); const aliveWerewolves=getAliveWerewolves(this.gameState.players);
    const candidates=alivePlayers.filter(p=>p.team!==Team.WEREWOLF);
    if(candidates.length===0) return {type:NightActionType.WOLF_KILL,targetId:this.player.id,actorId:this.player.id};
    const existing=this.gameState.nightActions.find(a=>a.type===NightActionType.WOLF_KILL&&a.actorId!==this.player.id);
    if(existing) return {type:NightActionType.WOLF_KILL,targetId:existing.targetId,actorId:this.player.id};
    const scores=new Map<number,number>();
    for(const c of candidates){ let s=1; for(const v of this.gameState.votes){ if(v.voterId===c.id){ const vp=findPlayerById(this.gameState.players,v.targetId); if(vp&&vp.role===Role.WEREWOLF) s+=5; } } s+=this.gameState.discussionLog.filter(d=>d.playerId===c.id).length; scores.set(c.id,s); }
    const sorted=Array.from(scores.entries()).sort((a,b)=>b[1]-a[1]); const top=sorted.filter(([,sc])=>sc===sorted[0][1]).map(([id])=>id);
    return {type:NightActionType.WOLF_KILL,targetId:pickRandom(top),actorId:this.player.id};
  }
  private decideSeerCheck():NightAction{
    const alive=getAlivePlayers(this.gameState.players).filter(p=>p.id!==this.player.id);
    if(alive.length===0) return {type:NightActionType.SEER_CHECK,targetId:this.player.id,actorId:this.player.id};
    const checked=this.player.seerChecks?.map(c=>c.targetId)||[]; const unchecked=alive.filter(p=>!checked.includes(p.id));
    const pool=unchecked.length>0?unchecked:alive; const target=pickRandom(pool);
    return {type:NightActionType.SEER_CHECK,targetId:target.id,actorId:this.player.id};
  }
  private decideGuardProtect():NightAction|null{
    if(!canGuardAct(this.player,this.gameState.day)) return null;
    ...
    if(specials.length>0) return {type:NightActionType.GUARD_PROTECT,targetId:pickRandom(specials),actorId:this.player.id};
    if(Math.random()<0.3) return {type:NightActionType.GUARD_PROTECT,targetId:this.player.id,actorId:this.player.id};
    const t=pickRandom(candidates); return {type:NightActionType.GUARD_PROTECT,targetId:t.id,actorId:this.player.id};
  }
  decideVote():number{
    const reasoning=this.buildReasoningContext(); const alive=getAlivePlayers(this.gameState.players); const cands=alive.filter(p=>p.id!==this.player.id);
    if(cands.length===0) return this.player.id;
    if(reasoning.suspectRanking.length>0){ const top=reasoning.suspectRanking[0]; if(cands.some(c=>c.id===top)) return top; }
  ...
  getReasoningContext():ReasoningContext{ return this.buildReasoningContext(); }
  private villagerVoteStrategy(cands:Player[]):number{
    const scores=new Map<number,number>(); for(const c of cands){ let s=1; s+=this.gameState.votes.filter(v=>v.targetId===c.id).length*2; s+=randomInt(0,3); scores.set(c.id,s); }
    const sorted=Array.from(scores.entries()).sort((a,b)=>b[1]-a[1]); const top=sorted.filter(([,s])=>s===sorted[0][1]).map(([id])=>id); return pickRandom(top);
  }
  private wolfVoteStrategy(cands:Player[]):number{
    const allies=getAliveWerewolves(this.gameState.players).map(w=>w.id).filter(id=>id!==this.player.id);
    const pool=cands.filter(c=>!allies.includes(c.id)); const usePool=pool.length>0?pool:cands;
    const scores=new Map<number,number>(); for(const c of usePool){ let s=1; for(const v of this.gameState.votes){ if(v.voterId===c.id){ const vp=findPlayerById(this.gameState.players,v.targetId); if(vp&&vp.role===Role.WEREWOLF) s+=10; } } scores.set(c.id,s); }
    const sorted=Array.from(scores.entries()).sort((a,b)=>b[1]-a[1]); const top=sorted.filter(([,s])=>s===sorted[0][1]).map(([id])=>id); return pickRandom(top);
  }
  private seerVoteStrategy(cands:Player[]):number{
    ...
    if(wolves.length>0) return wolves[0].targetId;
    return this.villagerVoteStrategy(cands);
  }
  private guardVoteStrategy(cands:Player[]):number{ return this.villagerVoteStrategy(cands); }
  private mediumVoteStrategy(cands:Player[]):number{ return this.villagerVoteStrategy(cands); }
