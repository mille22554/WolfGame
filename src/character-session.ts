...
export class CharacterSession {
   ...
   */
  async nightAction(): Promise<NightAction | null> {
    ...
    if (role === Role.WEREWOLF) {
      ...
      if (!this.isValidNightTarget(targetId)) return null;
      return { type: NightActionType.WOLF_KILL, targetId: targetId as number, actorId: this.playerId };
    }
    if (role === Role.SEER) {
      this.messages.push({
        role: 'user',
      ...
      const targetId = parseTarget(reply);
      if (!this.isValidNightTarget(targetId)) return null;
      return { type: NightActionType.SEER_CHECK, targetId: targetId as number, actorId: this.playerId };
    }
    if (role === Role.GUARD) {
      this.messages.push({
        role: 'user',
      ...
      const targetId = parseTarget(reply);
      if (!this.isValidNightTarget(targetId)) return null;
      return { type: NightActionType.GUARD_PROTECT, targetId: targetId as number, actorId: this.playerId };
    }
    return null;
  }

  /** 夜間目標必須是存活且非自己的玩家 */
  private isValidNightTarget(targetId: number | null): targetId is number {
    if (targetId === null || Number.isNaN(targetId)) return false;
...
function parseTarget(reply: string): number | null {
  const m = reply.match(/P(\d+)/);
  if (!m) return null;
  return parseInt(m[1], 10);
}
