...
import {
  createGameState, loadState, buildGMSnapshot, getNightActors,
} from './game-state.js';
...
function flush(engine: GameEngine, message?: string): void {
  engine.drain();
  engine.save();
  const snapshot = buildGMSnapshot(engine.getState());
  engine.close();
  if (message) output({ message, state: snapshot });
...
switch (command) {
  case 'init': {
    ...
    if (args[2]) {
      try {
        humans = JSON.parse(args[2]) as number[];
      } catch {
  ...
  case 'join': {
    const name = args[1];
    if (!name) error('Usage: gm.js join <name>');
  ...
  case 'state': {
    const state = loadState();
    if (!state) error('No game state file found.');
