...
import {
  ...
  collectWolfViolations, collectExpandViolations, findEmptyDiscussionFabrication,
  readPrecedents, appendPrecedents, findCrossGamePrecedent, buildCrossGameNote,
  findRecentPrecedentsByKind, PRECEDENTS_CAP, type PrecedentEntry,
    ...
    ctx: {
      ...
      enqueue: (e: GameEvent) => {
        ...
        if (e.type === 'AI_WOLF_READY') {
          if (!state.wolfReady.includes(e.playerId)) state.wolfReady.push(e.playerId);
...
test('全量記賬整合：違規稿記 N 筆同 text、棄權一次、無重試', async () => {
  ...
  try {
    ...
    sch.stop();
    try { fs.rmSync(file, { force: true }); } catch { /* 丟棄 */ }
  }
});
...
// 狼模式 expand 已移除（直接播出草稿）；以下 expand 測試僅適用於日模式
...
// 熔斷/文旗救回測試已移除：狼模式不再走 expand 管線

// expand 英文/簡體測試已移除：狼模式不再走 expand 管線
...
test('跨局每種一條：同 meeting＋phase 按 t 取各 kind 最新 1，上限 5 固定序', () => {
  const file = tmpLedger();
  try {
    const row = (t: number, kind: string, hit: string): PrecedentEntry => ({
...
test('跨局 lang 細分：簡體／英文各最新 1（英文不再被遮蔽）', () => {
  const file = tmpLedger();
  try {
    const row = (t: number, hit: string): PrecedentEntry => ({
...
// 狼模式 expand 決策優先/矛盾測試已移除：狼模式不再走 expand 管線
