...
import type { LobbyServerHandle } from './server.js';

interface TestClient {
  ws: WebSocket;
  /** 回傳下一則（已 parse 的）訊息；給 type 則等到該類型才 resolve。 */
...
function connect(port: number): Promise<TestClient> {
  return new Promise((resolve, reject) => {
    ...
    ws.on('open', () => {
      resolve({
        ws,
        next: (type?: string) => new Promise((res) => {
...
test('M1：建立房間＋加入（房主/成員視角）', async () => {
  ...
  const names = bJoined.players.map((p: any) => p.nickname);
